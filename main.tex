...
\begin{section}{\gls{bd}-\gls{ris} Model}
	...
	\begin{remark}
		By analyzing \eqref{eq:channel_equivalent_svd}, we conclude that channel shaping through \gls{bd}-\gls{ris} involves two critical challenges:
		\begin{itemize}
			% \item Subspace alignment: the singular matrices of backward and forward channels
			\item \textbf{Subspace alignment:} each group needs to balance the backward-forward (intra-group, multiplicative) and the direct-indirect (inter-group, additive) alignments of the singular matrices.
			\item \textbf{Subchannel rearrangement:} each group can rearrange and combine the backward and forward subchannels from strongest to weakest, which attains the maximal in rearrangement inequality.
		\end{itemize}
	\end{remark}
