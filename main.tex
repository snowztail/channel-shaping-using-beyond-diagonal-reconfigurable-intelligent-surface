...
\begin{abstract}
	...
	% the limit to which
	We depart from the conventional diagonal phase shift model and adopt a \gls{bd}-\gls{ris} architecture featuring element-wise connections for signal amplitude and phase control.
	...
	Specifically, we first provide
	shaping insights by characterizing the
	channel singular value region manipulated by diagonal and \gls{bd}-\gls{ris} via a novel geodesic manifold optimization.
	...
	analytical singular value bounds in special channel conditions.
	...
	As a side product, we tackle \gls{bd}-\gls{ris}-aided \gls{mimo} achievable rate maximization by a local-optimal \gls{ao} approach and a suboptimal shaping-inspired two-stage approach.
	...
	Compared to diagonal \gls{ris}, \gls{bd}-\gls{ris} significantly improves the dynamic range of all channel singular values and the trade-off in manipulating them.
	% , compared to conventional diagonal \gls{ris}.
	Results also highlight that the power and rate gain of \gls{bd}-\gls{ris} over diagonal \gls{ris} increase with the number of scattering elements and \gls{mimo} dimensions.
...
\begin{section}{Introduction}
	...
	\begin{subsection}{Related Works}
		...
		% On the other hand, the beamforming superiority of \gls{bd}-\gls{ris} over diagonal \gls{ris} has been extensively studied in \gls{siso} and \gls{miso} communication systems \cite{Shen2020a,Nerini2023,Nerini2024,Santamaria2023,Fang2023,Zhou2023,Li2023c,Soleymani2024}.
...
% 		% Those assumptions simplify the problem but limit the generality of the results.
	...
	% Extensive simulations show that the advantage of \gls{bd}-\gls{ris} stems from its superior space alignment and channel rearrangement capability, which scales with the number of elements, group size, \gls{mimo} dimensions, and channel diversity.
		...
		% Results show that \gls{bd}-\gls{ris} provides higher power gain and achievable rate than diagonal \gls{ris}, and the rate gap of two approaches diminishes as the \gls{bd}-\gls{ris} evolves from diagonal to fully-connected.
...
\begin{section}{\glsfmtshort{bd}-\glsfmtshort{ris} Model}
	...
	\begin{itemize}
		...
		% Diagonal \gls{ris} thus suffices
