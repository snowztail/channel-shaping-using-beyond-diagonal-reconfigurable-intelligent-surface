...
\begin{section}{Group-Wise Geodesic \glsfmtshort{rcg}}
	...
	\begin{subsection}{Geodesic vs Non-Geodesic \glsfmtshort{rcg}}
		...
		The main idea is to perform group-wise \emph{additive} updates along the conjugate direction [not in, but guided by] the tangent space, then project the solution back onto the manifold.
		...
		For maximization problem with smooth objective $f$ and block-unitary constraint \eqref{eq:bd_ris}, the steps for group $g$ at iteration $r$ are summarized below:
		\begin{enumerate}
			...
			\end{equation}
			\item \emph{Translate to the Riemannian gradient \cite{Absil2009,Abrudan2008}:} At point $\mathbf{\Theta}^{(r)}$, the Riemannian gradient lies in the tangent space of the Stiefel manifold $\mathcal{T}_{\mathbf{\Theta}_g^{(r)}}\mathbb{U}^{L \times L} \triangleq \{\mathbf{M} \mid \mathbf{M}^\mathsf{H} \mathbf{\Theta}_g^{(r)} + {\mathbf{\Theta}_g^{(r)}}^\mathsf{H} \mathbf{M} = \mathbf{0}\}$. It can be obtained by projecting the Euclidean gradient onto the tangent space:
			\begin{equation}
			...
			\end{equation}
			\item \emph{Determine the conjugate direction \cite{Nocedal2006,Abrudan2009}:} The conjugate direction is updated over the Riemannian gradient and previous direction as
			\begin{equation}
				\mathbf{D}_g^{(r)} = \nabla_{\mathrm{R},g}^{(r)} + \gamma_g^{(r)} \mathbf{D}_g^{(r-1)}, % $\mathcal{O}(N_\mathrm{S}^2)$
				\label{eq:direction_cg}
			\end{equation}
			where $\gamma_g^{(r)}$ is the deviation parameter that accelerates the convergence. One common choice is the Polak-Ribierre formula \cite{Hager2006}
			\begin{equation}
			...
			\end{equation}
			\item \emph{Update [aided by] the tangent space}
		\end{enumerate}
