...
\begin{abstract}
	...
	and adopt a \gls{bd} architecture not restricted to diagonal scattering matrix and featuring element-wise connections for extra wave manipulation flexibility.
	...
	Of particular interest, \gls{bd}-\gls{ris} is shown to activate multi-stream transmission at lower transmit power than \gls{d}-\gls{ris}, hence better approaching \gls{dof} at low \gls{snr} thanks to its higher flexibility of shaping the distribution of channel singular values.
...
% 		% Those assumptions simplify the problem but limit the generality of the results.
	...
	% Extensive simulations show that the advantage of \gls{bd}-\gls{ris} stems from its superior space alignment and channel rearrangement capability, which scales with the number of elements, group size, \gls{mimo} dimensions, and channel diversity.
		...
		Fifth, we tackle \gls{bd}-\gls{ris}-aided \gls{mimo} rate maximization problem by a local-optimal \gls{ao} and a low-complexity shaping-inspired approach.
...
\begin{section}{\glsfmtshort{bd}-\glsfmtshort{ris} Model}
	...
	\begin{itemize}
		...
		When it comes to \gls{mimo}, each \gls{d}-\gls{ris} element can only apply a scalar phase shift to the associated indirect channel matrix of size $N_\mathrm{R} \times N_\mathrm{T}$ defined in \eqref{eq:channel_equivalent}.
