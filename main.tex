...
\begin{section}{\glsentryshort{mimo}-\glsentryshort{pc}}
	...
	\begin{subsection}{Rate Maximization}
		...
		\end{remark}
		For general optimization problems with block unitary constraint, the adapted \gls{rcg} method at iteration $r$ for block $g$ is summarized below, where $f^{(r)}$ is evaluated over $\bigl\{ \{\mathbf{\Theta}_{g'}^{(r+1)}\}_{g'=1}^{g-1}, \{\mathbf{\Theta}_{g'}^{(r)}\}_{g'=g}^{G} \bigr\}$.
		\begin{enumerate}
			\item Compute the Euclidean gradient
			\begin{equation}
				\nabla_g^\mathrm{E} f^{(r)} = \frac{\partial f^{(r)}}{\partial \mathbf{\Theta}_g^*};
				\label{eq:rcg_gradient_euclidean}
			\end{equation}
			\item Translate to the Riemannian gradient
			\begin{equation}
				\nabla_g^\mathrm{R} f^{(r)} = \nabla_g^\mathrm{E} f^{(r)} {\mathbf{\Theta}_g^{(r)}}^\mathsf{H} - \mathbf{\Theta}_g^{(r)} {\nabla_g^\mathrm{E} f^{(r)}}^\mathsf{H};
				\label{eq:rcg_gradient_riemannian}
			\end{equation}
			\item Determine the weight factor
			\begin{equation}
				\gamma_g^{(r)} = \frac{\mathrm{tr}\Bigl(\bigl(\nabla_g^\mathrm{R} f^{(r)} - \nabla_g^\mathrm{R} f^{(r-1)}\bigr) \nabla_g^\mathrm{R} {f^{(r)}}^\mathsf{H}\Bigr)}{\mathrm{tr}\Bigl(\nabla_g^\mathrm{R} f^{(r-1)} \nabla_g^\mathrm{R} {f^{(r-1)}}^\mathsf{H}\Bigr)};
				\label{eq:rcg_weight}
			\end{equation}
			\item Compute the conjugate direction
			\begin{equation}
				\mathbf{D}_g^{(r)} = \nabla_g^\mathrm{R} f^{(r)} + \gamma_g^{(r)} \mathbf{D}_g^{(r-1)};
			...
			\begin{equation}
				\mu_g^{(r)} = \arg\max_{\mu_g} f^{(r)}\Bigl(\exp\bigl(\mu_g \mathbf{D}_g^{(r)}\bigr) \mathbf{\Theta}_g^{(r)}\Bigr);
				\label{eq:rcg_step}
			\end{equation}
		...
		\end{equation}
		The adapted \gls{rcg} method for the \gls{ris} rate maximization subproblem is summarized in Algorithm~\ref{ag:pc_rate_ris}.
		...
		\begin{algorithm}[!t]
			...
			\begin{algorithmic}[1]
				...
				% \State Initialize $r \gets 0$, $\mathbf{\Theta}^{(0)}$
				\State $r \gets 0$, $\mathbf{\Theta}^{(0)}$
				\Repeat
					% \State Update $r \gets r+1$
					\State $r \gets r+1$
					\For {$g \gets 1$ to $G$}
						% \State Get $\mathbf{\Theta}_g^{(r)}$ by \eqref{eq:pc_rate_gradient_ris}, \eqref{eq:rcg_gradient_riemannian} -- \eqref{eq:rcg_update}
