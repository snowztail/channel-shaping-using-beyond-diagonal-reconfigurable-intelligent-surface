...
\begin{abstract}
	...
	% \gls{ris} is a real-time programmable reflector that recycles and redistributes ambient waves for improved wireless performance.
	...
	% Through theoretical analysis and numerical evaluation,
	...
	% truly programmable
	% useful guidelines for prototype design.
\end{abstract}
...
\begin{section}{Introduction}
	...
	% , accounting for a diagonal phase shift model with unit magnitude.
	The idea is extended to \gls{bd} \gls{ris} \cite{Shen2020a,Li2023b} featuring in-group connections between elements.
	...
	When it comes to \gls{bd} \gls{ris}, the only metric analyzed is the channel power \cite{Nerini2023,Santamaria2023} while the \gls{mimo} case is still unexplored.\footnote{Single-stream \gls{mimo} with given precoder and combiner was considered in \cite{Nerini2023}. In terms of channel shaping, it is equivalent to \gls{siso}.}
	Although insightful, those attempts are far from complete and the methods are limited to specific scenarios.
	% This paper aims to quantify the channel shaping capability of asymmetric \gls{bd} \gls{ris} through theoretical analysis and numerical evaluation.
	...
	% quantify the channel shaping capability of asymmetric \gls{bd} \gls{ris} through theoretical analysis and numerical evaluation.
	...
	We then discuss analytical singular value bounds under rank-deficient and no-direct scenarios.

	% and demonstrate the channel shaping advantage of \gls{bd} \gls{ris} over diagonal \gls{ris}.

	Second, we tailor a \gls{rcg} algorithm for asymmetric\footnote{Although the constraint of symmetric scattering parameters is widely respected in the literature \cite{Shen2020a,Nerini2023,Santamaria2023,Fang2023,Nerini2023a,Zhou2023,Li2023d,Bartoli2023}, it can be relaxed when the \gls{bd} \gls{ris} involves asymmetric passive components \cite{Ahn2006}, as previously assumed in \cite{Li2023b,Li2023c}. Symmetry can be enforced by projection if necessary.} \gls{bd} \gls{ris} optimization.
	...
	% It is used to solve the Pareto singular problem, as well as rate maximization in \gls{mimo} \gls{pc} and \gls{wsr} maximization in \gls{mimo} \gls{ic}.
	% It effectively solves the Pareto singular problem.

	% , as well as rate maximization in \gls{mimo} \gls{pc} and \gls{wsr} maximization in \gls{mimo} \gls{ic}.

	% It helps to solve the aforementioned Pareto singular value problem.
	% We exploit it to solve the Pareto singular value problem and


	% It is exploited for the Pareto singular value problem, as well as rate maximization in \gls{mimo} \gls{pc} and \gls{wsr} maximization in \gls{mimo} \gls{ic}.
	% It is used to solve the Pareto singular value problem,

	Third, we consider power centric design
	% Second, we consider power centric design
	% exploit \gls{bd} \gls{ris} to power centric
	% maximize channel power in \gls{mimo} \gls{pc} and minimize leakage interference in \gls{mimo} \gls{ic}.
