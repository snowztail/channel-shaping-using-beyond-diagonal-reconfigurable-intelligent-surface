...
\begin{section}{Channel Singular Value Redistribution}
	...
	\begin{subsection}{Pareto Frontier Characterization}
		...
		Back to the Pareto channel singular value problem \eqref{op:pareto}. The Euclidean gradient of \gls{bd}-\gls{ris} group $g$ is computed as
		\begin{equation}
		...
		Algorithm~\ref{ag:rcg} can thus be invoked for the Pareto singular value problem \eqref{op:pareto}, where expression \eqref{eq:pareto_gradient} should be used in line \ref{ln:gradient_euclidean}.
	\end{subsection}
	...
	\begin{subsection}{Some Analytical Bounds}
		...
		We then discuss some analytical bounds related to channel singular values.
			% \gls{bd}-\gls{ris} cannot achieve a higher \gls{dof} than diagonal \gls{ris}.
			\begin{proposition}
				In rank-deficient point-to-point channel, \gls{bd}-\gls{ris} cannot achieve a higher \gls{dof} than diagonal \gls{ris}.
			\end{proposition}
			\begin{proof}
				...
				\end{equation}
				where $\mathbf{L}, \mathbf{R} \in \mathbb{U}^{N_\mathrm{S} \times N_\mathrm{S}}$ are block diagonal matrices of $L \times L$ unitary blocks and $\mathbf{\Theta}_\mathrm{D} \in \mathbb{U}^{N_\mathrm{S} \times N_\mathrm{S}}$ corresponds to diagonal \gls{ris}.
				% However, manipulating $\mathbf{L}$ and $\mathbf{R}$ does not change the rank of $\bar{\mathbf{H}}_\mathrm{B} \triangleq \mathbf{H}_\mathrm{B} \mathbf{L}$ and $\bar{\mathbf{H}}_\mathrm{F} \triangleq \mathbf{R}^\mathsf{H} \mathbf{H}_\mathrm{F}$.
				Manipulating $\mathbf{L}$ and $\mathbf{R}$ only rotates the linear spans of $\bar{\mathbf{H}}_\mathrm{B} \triangleq \mathbf{H}_\mathrm{B} \mathbf{L}$ and $\bar{\mathbf{H}}_\mathrm{F} \triangleq \mathbf{R}^\mathsf{H} \mathbf{H}_\mathrm{F}$ and keeps their rank.
				On the other hand, there exists
				\begin{equation}
					\arg_{\mathbf{\Theta}_\mathrm{D}} \max \ \mathrm{rank}(\mathbf{H}_\mathrm{B} \mathbf{\Theta}_\mathrm{D} \mathbf{H}_\mathrm{F}) = \min \bigl( \mathrm{rank}(\bar{\mathbf{H}}_\mathrm{B}), \mathrm{rank}(\mathbf{\Theta}_\mathrm{D}), \mathrm{rank}(\bar{\mathbf{H}}_\mathrm{F}) \bigr).
				\end{equation}
				% Therefore,
				% \begin{align}
				% 	\mathrm{rank}(\mathbf{H}_\mathrm{B} \mathbf{\Theta} \mathbf{H}_\mathrm{F}) & = \mathrm{rank}(\mathbf{H}_\mathrm{B} \mathbf{L} \mathbf{\Theta}_\mathrm{D} \mathbf{R}^\mathsf{H} \mathbf{H}_\mathrm{F}) \\
				% 	& = \mathrm{rank}(\bar{\mathbf{H}}_\mathrm{B} \mathbf{\Theta}_\mathrm{D} \bar{\mathbf{H}}_\mathrm{F}) \\
				% 	& \le \min \bigl( \mathrm{rank}(\bar{\mathbf{H}}_\mathrm{B}), \mathrm{rank}(\mathbf{\Theta}_\mathrm{D}), \mathrm{rank}(\bar{\mathbf{H}}_\mathrm{F}) \bigr).
				% \end{align}
				% On the other hand, we have
				% \begin{align}
				% 	\mathrm{rank}(\mathbf{H}_\mathrm{B} \mathbf{\Theta} \mathbf{H}_\mathrm{F})
				% \end{align}
				% Therefore,
				% \begin{align}
				% 	& \arg_\mathbf{\Theta} \max \ \mathrm{rank}(\mathbf{H}_\mathrm{B} \mathbf{\Theta} \mathbf{H}_\mathrm{F}) \\
				% 	= & \arg_{\mathbf{\Theta}_\mathrm{D},\mathbf{L},\mathbf{R}} \max \ \mathrm{rank}(\bar{\mathbf{H}}_\mathrm{B} \mathbf{\Theta}_\mathrm{D} \bar{\mathbf{H}}_\mathrm{F}) \\
				% 	= & \arg_{\mathbf{L},\mathbf{R}} \max \  \min \bigl( \mathrm{rank}(\bar{\mathbf{H}}_\mathrm{B}), \mathrm{rank}(\mathbf{\Theta}_\mathrm{D}), \mathrm{rank}(\bar{\mathbf{H}}_\mathrm{F}) \bigr) \\
				% 	= & \min \bigl( \mathrm{rank}(\mathbf{H}_\mathrm{B}), \mathrm{rank}(\mathbf{\Theta}_\mathrm{D}), \mathrm{rank}(\mathbf{H}_\mathrm{F}) \bigr).
				% \end{align}
				% That is, $\mathrm{rank}(\mathbf{H}_\mathrm{B} \mathbf{\Theta} \mathbf{H}_\mathrm{F}) = \mathrm{rank}(\mathbf{H}_\mathrm{B} \mathbf{\Theta}_\mathrm{D} \mathbf{H}_\mathrm{F})$
				% That is, $\mathrm{rank}(\mathbf{H}) = \mathrm{rank}(\mathbf{H}_\mathrm{D}) + \mathrm{rank}(\bar{\mathbf{H}}_\mathrm{B} \mathbf{\Theta}_\mathrm{D} \bar{\mathbf{H}}_\mathrm{F})$.
				% When direct link is present,
				% does not change the rank of $\bar{\mathbf{H}}_\mathrm{B} \triangleq \mathbf{H}_\mathrm{B} \mathbf{L}$ and $\bar{\mathbf{H}}_\mathrm{F} \triangleq \mathbf{R}^\mathsf{H} \mathbf{H}_\mathrm{F}$.
				% Although \gls{bd}-\gls{ris} can
				% It follows straightforwardly from the rank inequality
				% \begin{equation}
				% 	\begin{split}
				% 		\mathrm{rank}(\mathbf{H})
				% 		& \le \mathrm{rank}(\mathbf{H}_\mathrm{D}) + \mathrm{rank}(\mathbf{H}_\mathrm{B} \mathbf{\Theta} \mathbf{H}_\mathrm{F}) \\
				% 		& \le \mathrm{rank}(\mathbf{H}_\mathrm{D}) + \min \bigl( \mathrm{rank}(\mathbf{H}_\mathrm{B}), \mathrm{rank}(\mathbf{\Theta}), \mathrm{rank}(\mathbf{H}^\mathrm{F}) \bigr).
				% 	\end{split}
				% \end{equation}
				% % The \gls{dof} upper bound of \gls{bd}-\gls{ris} $\mathbf{\Theta}_\mathrm{B}$ and diagonal \gls{ris} $\mathbf{\Theta}_\mathrm{D}$ coincide as they are both full-rank with repeating singular value 1.
				% The \gls{dof} upper bound of \gls{bd}-\gls{ris} and diagonal \gls{ris} coincide because their scattering matrices are both full-rank (with all singular values equal to 1).
				% On the other hand, diagonal \gls{ris} can already achieve the maximum \gls{dof} $\max(N, U)$

				% ?
				% \gls{bd}-\gls{ris} and diagonal \gls{ris} are both full-rank with all singular values equal 1, so their \gls{dof} upper bound coincide.
				% On the other hand, a diagonal $\mathrm{span}(\mathbf{H}_\mathrm{D}) \cap \mathrm{span}(\mathbf{H}_\mathrm{B} \mathbf{\Theta} \mathbf{H}_\mathrm{F}) = \varnothing$
				% On the other hand, a diagonal $\mathrm{span}(\mathbf{H}_\mathrm{D}) \cap \mathrm{span}(\mathbf{H}_\mathrm{B} \mathbf{\Theta} \mathbf{H}_\mathrm{F}) = \varnothing$
				% diagonal \gls{ris} can already achieve $\mathrm{span}(\mathbf{H}_\mathrm{D}) \cap \mathrm{span}(\mathbf{H}_\mathrm{B} \mathbf{\Theta} \mathbf{H}_\mathrm{F}) = \varnothing$ by designing the diagonal phase shift matrix.
				% $\mathrm{span}(\mathbf{H}_\mathrm{D}) \cap \mathrm{span}(\mathbf{H}_\mathrm{B} \mathbf{\Theta} \mathbf{H}_\mathrm{F}) = \varnothing$
				% $\mathrm{span}(\mathbf{H}_\mathrm{D})$ and $\mathrm{span}(\mathbf{H}_\mathrm{B} \mathbf{\Theta} \mathbf{H}_\mathrm{F})$ can be made disjoint by choosing $\mathbf{\Theta}$ properly, so the \gls{dof} lower bound of \gls{bd}-\gls{ris} is no larger than that of diagonal \gls{ris}.
				% are disjoint, so the \gls{dof} lower bound of \gls{bd}-\gls{ris} is no larger than that of diagonal \gls{ris}.
				% the linear spans of $\mathbf{H}_\mathrm{D}$ and $\mathbf{H}_\mathrm{B} \mathbf{\Theta} \mathbf{H}_\mathrm{F}$ are disjoint, so the \gls{dof} lower bound of \gls{bd}-\gls{ris} is no larger than that of diagonal \gls{ris}.
				% using diagonal \gls{ris} can already achieve the upper bound.
			\end{proof}
