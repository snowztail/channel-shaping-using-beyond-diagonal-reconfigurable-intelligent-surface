...
\begin{section}{\gls{bd}-\gls{ris} Model}
	...
	Some potential physical architectures of \gls{bd}-\gls{ris} are illustrated in \cite[Fig. 3]{Shen2020a}, \cite[Fig. 5]{Li2023c}, and \cite[Fig. 2]{Nerini2023a}, where the radiation pattern and circuit topology have appreciable impacts on the scattering matrix.
	...
	Since unitary matrices constitute a field with respect to multiplication, the scattering matrix of group $g$ can be decomposed as
	\begin{equation}
	...
	\end{equation}
	Clearly, channel shaping involves two critical issues:
	% From \eqref{eq:channel_equivalent_svd}, we can see that channel shaping via \gls{ris} involves two critical aspects:
	\begin{itemize}
		\item \emph{Subspace alignment:} there exists a tradeoff between the additive (direct-indirect) and multiplicative (backward-forward) alignments of channel singular matrices;
		% group $g$ can completely align the subspaces of backward and forward subchannel
		\item \emph{Subchannel rearrangement:} the backward and forward subchannels associated with any group can be rearranged and combined.
		% , which grants better performance due to the rearrangement inequalities \cite{Zhao2022}.
	\end{itemize}
	% types of \emph{subspace alignment} problems: direct-indirect (additive) and backward-forward (multiplicative).
	% which involves two types of \emph{subspace alignment} problems: direct-indirect (additive) and backward-forward (multiplicative).
