	...
	% We also provide some analytical bounds related to channel singular values, which help us to explore the limits of channel shaping using passive \gls{ris}.
		...
		\begin{corollary}[Individual singular value\label{co:sv_nth}]
			...
			\begin{itemize}
				...
				% 		\underset{i+j=n+\bar{N}}{\arg\max} \sigma_i(\mathbf{H}_\mathrm{B}) \sigma_j(\mathbf{H}_\mathrm{F}) & \text{for } \mathbf{Q},
				% 	\end{cases}
				% \end{equation*}
				\begin{subnumcases}{(i, j) =}
					...
					\underset{i+j=n+\bar{N}}{\arg\max} \sigma_i(\mathbf{H}_\mathrm{B}) \sigma_j(\mathbf{H}_\mathrm{F}) & for $\mathbf{Q}$, \label{eq:sv_nth_min_pair}
				\end{subnumcases}
				% \begin{equation*}
				% 	(i, j) =
				% 	\begin{cases}
				...
				% 		\sigma_{n{+}1}(\hat{\mathbf{\Sigma}}_{\mathrm{B}} \mathbf{Y} \hat{\mathbf{\Sigma}}_{\mathrm{F}}) {\le} \max_{i+j=n+\bar{N}} \sigma_i(\mathbf{H}_\mathrm{B}) \sigma_j(\mathbf{H}_\mathrm{F}) & \text{ for } \mathbf{Q}, \hspace{-0.17cm} \label{eq:sv_nth_min_condition}
				% 	\end{alignat}
				% \end{subequations}
