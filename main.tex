...
\begin{section}{\gls{bd}-\gls{ris} Model}
	...
	\end{remark}
	\begin{remark}
		Individual forward and backward \gls{csi} are generally required for \gls{bd}-\gls{ris} designs.
...
\begin{section}{Channel Singular Value Redistribution}
	\begin{subsection}{A Toy Example}
		% We first show the channel shaping advantage of \gls{bd}-\gls{ris} by a toy example.
		...
		The diagonal \gls{ris} is modeled by $\mathbf{\Theta}_\mathrm{D} = \mathrm{diag}(e^{\jmath \theta_1}, e^{\jmath \theta_2})$ while the unitary \gls{ris} has 4 independent angular parameters
		\begin{equation}
		...
		A larger performance gap is expected for asymmetric \gls{bd}-\gls{ris}.
		This example emphasizes the importance of cooperative scattering and motivates further studies on channel shaping.
