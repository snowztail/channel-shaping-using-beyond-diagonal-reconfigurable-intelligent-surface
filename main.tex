...
\begin{section}{Group-Wise Geodesic \glsfmtshort{rcg}}
	...
	\begin{subsection}{Geodesic vs Non-Geodesic \glsfmtshort{rcg}}
		...
		Note that the set of unitary matrices $\mathbb{U}^{L \times L}$ forms a Lie group $U(L)$ under multiplication, and the corresponding Lie algebra $\mathfrak{u}(L) \triangleq \mathcal{T}_{\mathbf{I}}\mathbb{U}^{L \times L} = \{\mathbf{M} \in \mathbb{C}^{L \times L} \mid \mathbf{M}^\mathsf{H} + \mathbf{M} = \mathbf{0}\}$ consists of skew-Hermitian matrices.
		...
		Since block-unitary matrices are also closed under multiplication, one can avoid group-wise updates by operating directly on $\mathbf{\Theta}$ and pinching (i.e., keeping the block diagonal and nulling the other entries) the Euclidean gradient \eqref{eq:gradient_euclidean}, at the cost of higher computational complexity and slower convergence.
	\end{subsection}
\end{section}
