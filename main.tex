...
\begin{section}{Rate Maximization}
	...
	\begin{subsection}{Low-Complexity Solution}
		...
		\begin{proposition}
			...
			\end{equation}
			converges to a stationary point of \eqref{op:pc_power}, where $\mathbf{U}_g^{(r)}$ and $\mathbf{V}_g^{(r)}$ are the left and right compact singular matrix of
			\begin{equation}
		...
		\begin{proof}
			...
			% As for the stat
			% The Lagrangian of problem \eqref{op:power_passive} and \eqref{op:power_passive_approx} are respectively
			% \begin{align}
			% 	\mathcal{L}(\mathbf{\Theta},\mathbf{\Lambda}) & = \sum_g 2 \Re \bigl\{\mathrm{tr}(\mathbf{\Theta}_g^\mathsf{H} \mathbf{M}_g)\bigr\} + \sum_g \mathrm{tr} \bigl(\mathbf{\Lambda}_g^\mathsf{H} (\mathbf{\Theta}_g^\mathsf{H} \mathbf{\Theta}_g - \mathbf{I})\bigr),\\
			% 	\mathcal{L}(\mathbf{\Theta},\mathbf{\Lambda}) & = \lVert \mathbf{H}_\mathrm{D} + \mathbf{H}_\mathrm{B} \mathbf{\Theta} \mathbf{H}_\mathrm{F} \rVert _\mathrm{F}^2 + \sum_g \mathrm{tr} \bigl(\mathbf{\Lambda}_g^\mathsf{H} (\mathbf{\Theta}_g^\mathsf{H} \mathbf{\Theta}_g - \mathbf{I})\bigr),
			% \end{align}
			% That is, we only need to show the stationary condition of \eqref{op:power_passive} is satisfied by $\tilde{\mathbf{\Theta}}^?$.
			% Let $\tilde{\mathbf{\Theta}}^?$ be the converging point of \eqref{op:power_passive_approx}, which satisfies the \gls{kkt} conditions



			% The Lagrangian of problem \eqref{op:power_passive} is
			% \begin{equation}
			% 	\mathcal{L}(\mathbf{\Theta},\mathbf{\Theta}^*) = \lVert \mathbf{H}_\mathrm{D} + \mathbf{H}_\mathrm{B} \mathbf{\Theta} \mathbf{H}_\mathrm{F} \rVert _\mathrm{F}^2 + \sum_g \mathrm{tr} \bigl(\mathbf{\Lambda}_g^\mathsf{H} (\mathbf{\Theta}_g^\mathsf{H} \mathbf{\Theta}_g - \mathbf{I})\bigr),
			% \end{equation}
		\end{proof}
	\end{subsection}
