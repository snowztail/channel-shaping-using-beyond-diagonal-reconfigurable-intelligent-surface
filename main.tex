...
\begin{abstract}
	...
	We depart from the widely-adopted diagonal phase shift model to a \gls{bd} architecture, which enables {cooperative} wave scattering thanks to in-group connections between elements.
	...
	% For the singular value shaping problem, we numerically characterize their Pareto frontiers by weighted sum optimization via geodesic \gls{rcg}, which generalizes most relevant metrics (e.g., degree of freedom and condition number).
	% Many analytical singular value bounds are also derived for rank-deficient channels and unitary (a.k.a. unitary) \gls{ris}.
	For the singular value problem, we numerically characterize their Pareto frontiers by geodesic \gls{rcg} and derive analytical bounds for rank-deficient channels and fully-connected (a.k.a. unitary) \gls{ris}.
	% For the singular value problem, we consider a numerical optimization approach that characterizes the Pareto frontiers as well as an analytical bounding approach in special (i.e., rank-deficient and fully-connected) scenarios.
	...
	% Those channel shaping designs have a two-fold advantage.
	...
	Simulation results indicate that the rate deficit from the latter diminishes as the \gls{ris} evolves from diagonal towards unitary, while the relative gain of \gls{bd}-\gls{ris} increase with \gls{mimo} dimensions and decrease with spatial correlation.
...
\begin{section}{Group-Wise Geodesic \glsfmtshort{rcg}}
	...
	\begin{subsection}{Geodesic vs Non-Geodesic \glsfmtshort{rcg}}
		...
		The main idea is to perform \emph{additive} updates along the conjugate direction guided by the Riemannian gradient, then project the solution back onto the manifold.
		...
		\begin{enumerate}
			...
			\end{equation}
			\item \emph{Translate to the Riemannian gradient \cite{Absil2009}:} At point $\mathbf{\Theta}^{(r)}$, the Riemannian gradient lies in the tangent space of the Stiefel manifold $\mathcal{T}_{\mathbf{\Theta}_g^{(r)}}\mathbb{U}^{L \times L} \triangleq \{\mathbf{M} \mid \mathbf{M}^\mathsf{H} \mathbf{\Theta}_g^{(r)} + {\mathbf{\Theta}_g^{(r)\mathsf{H}}} \mathbf{M} = \mathbf{0}\}$. It gives the steepest ascent direction of the objective on the manifold can be obtained by projecting the Euclidean gradient onto the tangent space:
			\begin{equation}
			...
			\end{equation}
			where $\gamma$ is the step size refinable by the Armijo rule \cite{Armijo1966};
		...
		% On the other hand, a geodesic is a curve representing the shortest path between two points in a Riemannian manifold.
		% Its tangent vectors remain parallel when transported along the curve and the acceleration vectors stay normal to the motion.
		Since unitary matrices forms a Lie group that is closed under multiplication and inversion, and the corresponding geodesic emanating from $\mathbf{\Theta}_g$ with a velocity matrix $\mathbf{S}$ can be described by \cite{Edelman1998}
		\begin{equation}
			\mathbf{G}_g = \exp(t \mathbf{S}) \mathbf{\Theta}_g,
		\end{equation}


		% Lie groups are closed under multiplication and inversion, and the set of unitary matrices $\mathbb{U}^{L \times L}$ forms a Lie group.
		% The set of unitary matrices $\mathbb{U}^{L \times L}$ forms a Lie group that is closed under multiplication and inversion.
		% Since unitary matrices in the Stiefel manifold $\mathbb{U}^{L \times L}$ constitute a Lie group, the corresponding geodesic emanating from $\mathbf{\Theta}_g$ with velocity $\mathbf{S}$ is described by the exponential map \cite{Edelman1998}
		% \begin{equation}
		% 	\mathbf{G}_g = \exp(t \mathbf{S}) \mathbf{\Theta}_g,
		% \end{equation}
		% $\mathbf{\Theta}_g^{(r)}$ with velocity $\mathbf{D}_g^{(r)}$ and step size $\mu$ is described compactly by the exponential map \cite{Edelman1998}


		% A Lie group is a group that is also a differentiable manifold.
		% The Lie group of unitary matrices $\mathbb{U}^{L \times L}$ is a manifold that is closed under multiplication and inversion.
		% A geodesic is a curve representing the shortest path between two points in a Riemannian manifold.
		% locally minimizes the distance between two points in a Riemannian manifold.

		% Inspired by \cite{Abrudan2008,Abrudan2009}, we propose a group-wise \gls{rcg} algorithm along the geodesics on the Lie group of unitary matrices $\mathbb{U}^{L \times L}$.

		% It is worth noticing that the feasible domain


		% It is worth noting that the objective \eqref{ob:pareto} is smooth in $\mathbf{\Theta}$ and the feasible domain \eqref{co:pareto_unitary} corresponds to the Stiefel manifold for group $g$.

		% Both properties are often satisfied in \gls{bd}-\gls{ris} optimization problems, and there exists two general solutions in the literature:
		% \begin{itemize}
		% 	\item \emph{Quasi-Newton method \cite{Shen2020a}:}
		% 	% This is a general-purpose optimization algorithm that approximates the Hessian matrix by the gradient and iteratively updates the scattering matrix along the negative gradient direction.
		% \end{itemize}


		% have exploited either the quasi-Newton method on the reactance
		% Instead of solving it directly, we zoom out to general \gls{bd}-\gls{ris} optimization problem and propose an efficient and universal solution.
		...
		% 	\label{eq:direction_cg}
		% \end{equation}
		on the Stiefel manifold, the geodesic emanating from $\mathbf{\Theta}_g^{(r)}$ with velocity $\mathbf{D}_g^{(r)}$ and step size $\mu$ is described compactly by the exponential map \cite{Edelman1998}
		\begin{equation}
		...
		It is worth mentioning that the updates can be performed in parallel for all groups, which is particularly suitable for large-scale \gls{bd}-\gls{ris} optimization problems.
		One can also avoid group-wise updates by treating the entire scattering matrix as a single optimization variable, but this may lead to higher computational complexity and slower convergence.



		Algorithm~\ref{ag:rcg} summarizes the proposed group-wise geodesic \gls{rcg} method for smooth maximization problems of asymmetric \gls{bd}-\gls{ris}.
		...
		\begin{algorithm}[!t]
			\small
			\caption{Group-wise geodesic \gls{rcg} for asymmetric \gls{bd}-\gls{ris}}
			\label{ag:rcg}
			\begin{algorithmic}[1]
...
\begin{section}{Channel Singular Values Redistribution}
	\begin{subsection}{Toy Example}\label{sc:toy_example}
		We first illustrate the channel shaping capabilities of different \gls{ris} architectures by a toy example.
	...
	\begin{subsection}{Pareto Frontier Characterization}
		...
		Thus, we claim \eqref{op:pareto} generalizes most relevant channel shaping problems.
		% It is worth noting that the objective \eqref{ob:pareto} is smooth in $\mathbf{\Theta}$ and the feasible domain \eqref{co:pareto_unitary} corresponds to the Stiefel manifold for group $g$.
...
\begin{section}{Achievable Rate Maximization}\label{sc:rate}
	...
	\begin{subsection}{Low-Complexity Solution}\label{sc:low_complexity}
		...
		\begin{remark}
			...
			This suboptimal solution only applies to unitary \gls{bd}-\gls{ris}.
		\end{remark}
	...
	% When the direct link is absent, only \qty{2.6}{\percent} of available power is utilized by diagonal \gls{ris} while \qty{100}{\percent} power is recycled by unitary \gls{ris}.
...
\begin{section}{Simulation Results}\label{sc:simulation}
	...
	$\kappa = 0$ is assumed for all channels unless otherwise specified.
		...
		\begin{subsubsection}{Analytical Bounds and Numerical Results}
			\begin{figure}[!t]
				...
				\caption{
					...
					`D' means diagonal \gls{ris} and `BD' means unitary \gls{bd}-\gls{ris}.
			...
			Here, a unitary \gls{bd}-\gls{ris} can attain the upper bound either in closed form \eqref{eq:scattering_fc_tight} or via optimization approach \eqref{eq:scattering_power}.
			% Recall that a unitary \gls{bd}-\gls{ris} in closed form \eqref{eq:scattering_fc_tight} can attain the bound.
			% We observe that the bound is also tight for unitary \gls{bd}-\gls{ris}.
			% Increasing the group size of \gls{bd}-\gls{ris} significantly improves the channel power especially for a large \gls{mimo}.
			% Apparently, \gls{bd}-\gls{ris} with a larger group size can approach the bound better, and a unitary one can achieve the bound.
			...
			% We observe that the bound is tight for unitary \gls{bd}-\gls{ris} in \gls{siso} and \gls{mimo}.
			For the \gls{siso} case in Fig. \subref*{fg:power_bond_txrx1_nd}, the maximum channel power is approximately \num{4e-6} by diagonal \gls{ris} and \num{6.5e-6} by unitary \gls{bd}-\gls{ris}, corresponding to a \qty{62.5}{\percent} gain.
			This aligns with the asymptotic \gls{bd}-\gls{ris} scaling law derived for \gls{siso} in \cite{Shen2020a}.
			Interestingly, the gain surges to \qty{270}{\percent} in 4T4R \gls{mimo} as shown in Fig. \subref*{fg:power_bond_txrx4_nd}.
			This is because subspace alignment boils down to phase matching in \gls{siso} such that both triangular and Cauchy-Schwarz inequalities in \cite[(50)]{Shen2020a} can be simultaneously tight regardless of the group size.
			That is, diagonal \gls{ris} is sufficient for subspace alignment in \gls{siso} while the \qty{62.5}{\percent} gain from \gls{bd}-\gls{ris} comes purely from subchannel rearrangement (i.e., pairing the forward and backward channels from strongest to weakest).
	...
	\begin{subsection}{Achievable Rate Maximization}
		...
		We also notice that the suboptimal solutions \eqref{eq:orthogonal_procrustes_solution} for unitary \gls{bd}-\gls{ris} by lossy transformation \eqref{op:standard_orthogonal_procrustes} are very close to optimal especially for a large $N_\mathrm{S}$.
		...
		Interestingly, the gap between the optimal \gls{ao} approach \eqref{op:rate_passive}--\eqref{eq:precoder_eigenmode} and the low-complexity solution \eqref{eq:scattering_power} and \eqref{eq:precoder_eigenmode} narrows as the group size increases, and completely vanishes for a unitary \gls{bd}-\gls{ris}.
