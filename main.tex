...
\begin{abstract}
	...
	An efficient and universal \gls{bd}-\gls{ris} design framework is proposed over group-wise {geodesic} \gls{rcg} in the Stiefel manifold, which performs multiplicative (instead of add-then-retract) updates along the exponential map for faster convergence.
	...
	For the singular value problem, we characterize their Pareto frontiers by geodesic \gls{rcg} and derive analytical bounds for rank-deficient channels and fully-connected (a.k.a. unitary) \gls{ris}.
...
\begin{section}{Introduction}
	...
	% Specifically,  we show that BD-RIS enables cooperative groups to rearrange subchannels according to their strengths, maximizing the effective channel gain. Additionally, intra-group cooperation allows for finer control over subspace alignment compared to diagonal RIS, especially in higher-dimensional MIMO systems. This analysis highlights a key advantage of BD-RIS and its potential for significant performance improvement.
