...
\begin{section}{Analytical Bounds on Singular Value Shaping}
	...
	% We also provide some analytical bounds related to channel singular values, which help us to explore the limits of channel shaping using passive \gls{ris}.
		...
		% ! For example, the number of inequalities described by \eqref{iq:sv_bound_fc} grows from 12 to 2062 when $N_\mathrm{S}$ increases from 3 to 7.
		...
		\begin{corollary}[Individual singular value\label{co:sv_nth}]
			...
			\begin{itemize}
				...
				\begin{subnumcases}{(i, j) =}
					\underset{i+j=n+1}{\arg\min} \sigma_i(\mathbf{H}_\mathrm{B}) \sigma_j(\mathbf{H}_\mathrm{F}) & for $\mathbf{P}$, \label{eq:sv_nth_max_pair} \\
					\underset{i+j=n+N_\mathrm{S}}{\arg\max} \sigma_i(\mathbf{H}_\mathrm{B}) \sigma_j(\mathbf{H}_\mathrm{F}) & for $\mathbf{Q}$, \label{eq:sv_nth_min_pair}
				\end{subnumcases}
				% \begin{equation*}
				% 	(i, j) =
				% 	\begin{cases}
				...
				% and the tie-breaking rule is arbitrary.
				...
				% \begin{subequations}
				% 	\begin{alignat}{1}
				% 		\sigma_{n{-}1}(\hat{\mathbf{\Sigma}}_{\mathrm{B}} \mathbf{Y} \hat{\mathbf{\Sigma}}_{\mathrm{F}}) {\ge} \min_{i+j=n+1} \sigma_i(\mathbf{H}_\mathrm{B}) \sigma_j(\mathbf{H}_\mathrm{F}) & \text{ for } \mathbf{P}, \hspace{-0.17cm} \label{eq:sv_nth_max_condition} \\
				% 		\sigma_{n{+}1}(\hat{\mathbf{\Sigma}}_{\mathrm{B}} \mathbf{Y} \hat{\mathbf{\Sigma}}_{\mathrm{F}}) {\le} \max_{i+j=n+N_\mathrm{S}} \sigma_i(\mathbf{H}_\mathrm{B}) \sigma_j(\mathbf{H}_\mathrm{F}) & \text{ for } \mathbf{Q}, \hspace{-0.17cm} \label{eq:sv_nth_min_condition}
				% 	\end{alignat}
				% \end{subequations}
				...
				% \begin{subequations}
				% 	\begin{align}
				% 		\sigma_{n-1}(\hat{\mathbf{\Sigma}}_{\mathrm{B}} \mathbf{Y} \hat{\mathbf{\Sigma}}_{\mathrm{F}}) & \ge \min_{i+j=n+1} \sigma_i(\mathbf{H}_\mathrm{B}) \sigma_j(\mathbf{H}_\mathrm{F}) \text{for } \mathbf{P}, \label{eq:sv_nth_max_condition} \MoveEqLeft[1] \\
				% 		\sigma_{n+1}(\hat{\mathbf{\Sigma}}_{\mathrm{B}} \mathbf{Y} \hat{\mathbf{\Sigma}}_{\mathrm{F}}) & \le \max_{i+j=n+N_\mathrm{S}} \sigma_i(\mathbf{H}_\mathrm{B}) \sigma_j(\mathbf{H}_\mathrm{F}) \text{for } \mathbf{Q}, \label{eq:sv_nth_min_condition}
				% 	\end{align}
				% \end{subequations}
		...
		% Although Corollary \ref{co:sv_nth} and Proposition \ref{pp:direct_negligible}
		...
		% from a singular value perspective (discussed in Section~\ref{sc:pareto_frontier}) or a power gain perspective (to be discussed in Section~\ref{sc:power}).
...
\begin{appendix}
	...
	\begin{subsection}{Proof of Corollary \ref{co:sv_nth}}\label{ap:sv_nth}
		...
		% 	\label{iq:sv_nth_upper_set}
		% \end{equation}
		% is a special case of \eqref{iq:sv_bound_fc} with $(I, J, K) \in [N]^3$.
		% The minimum\footnote{One may think to take the \emph{maximum} of those upper bounds as the problem of interest is the attainable dynamic range of $n$-th singular value. However, this is infeasible since the singular values will be reordered therein.} of \eqref{iq:sv_nth_upper_set} is selected as the tightest upper bound in \eqref{iq:sv_nth}.
		% On the other hand, the set of lower bounds
		% \begin{equation}
