...
\begin{section}{Rate Maximization}
	...
	\begin{subsection}{Alternating Optimization}
		...
		\end{proof}
		% Problem \eqref{op:rate_passive} is then solved by Algorithm \ref{ag:rcg} where line \ref{ln:gradient_euclidean} uses \eqref{eq:rate_gradient} explicitly.
		% Problem \eqref{op:rate_passive} is then solved by Algorithm \ref{ag:rcg} where line \ref{ln:gradient_euclidean} uses \eqref{eq:rate_gradient} explicitly.
		...
		% is a first-order method to \eqref{op:rate_passive} and the water-filling algorithm is a global optimal solution to the convex problem \eqref{op:rate}.
	\end{subsection}
	...
	\begin{subsection}{Low-Complexity Solution}
		...
		% approximate the quadratic objective \eqref{ob:power_passive} with a sequence of affines and solve each problem
		% iterative algorithm to solve \eqref{op:pc_power}.
		...
		\begin{proposition}
			...
			\begin{equation}
				% \mathbf{M}_g^{(r)} = \mathbf{H}_{\mathrm{B},g}^\mathsf{H} (\mathbf{H}_\mathrm{D} + \mathbf{H}_\mathrm{B} \mathbf{\Phi}_g^{(r)} \mathbf{H}_\mathrm{F}) \mathbf{H}_{\mathrm{F},g}^\mathsf{H}
				% \mathbf{M}_g^{(r)} = \mathbf{H}_{\mathrm{B},g}^\mathsf{H} (\mathbf{H}_\mathrm{D} + \mathbf{H}_\mathrm{B} \mathrm{diag}(\mathbf{\Theta}_1^{(r+1)},\ldots,\mathbf{\Theta}_{g-1}^{(r+1)},\mathbf{\Theta}_g^{(r)},\ldots,\mathbf{\Theta}_G^{(r)}) \mathbf{H}_\mathrm{F}) \mathbf{H}_{\mathrm{F},g}^\mathsf{H}
				...
				% \mathbf{M}_g^{(r)} = \mathbf{H}_{\mathrm{B},g}^\mathsf{H} \mathbf{H}_g^{(r)} \mathbf{H}_{\mathrm{F},g}^\mathsf{H},
			\end{equation}
			% where $\mathbf{H}_g^{(r)} \triangleq \mathbf{H}_\mathrm{B} \mathrm{diag}(\mathbf{\Theta}_1^{(r+1)},\ldots,\mathbf{\Theta}_{g-1}^{(r+1)},\mathbf{\Theta}_g^{(r)},\ldots,\mathbf{\Theta}_G^{(r)}) \mathbf{H}_\mathrm{F}$.
			% \begin{equation}
			% 	\begin{split}
			% 		\mathbf{M}_g^{(r)}
