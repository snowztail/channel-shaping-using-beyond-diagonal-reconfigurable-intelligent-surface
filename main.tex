...
\begin{abstract}
	...
	and adopt a \gls{bd} architecture featuring element-wise connections for signal amplitude and phase control.
	...
	As a side product, we tackle \gls{bd}-\gls{ris}-aided \gls{mimo} rate maximization problem by a local-optimal \gls{ao} and then propose a shaping-inspired low-complexity approach.
	...
	Results show that compared to \gls{d}-\gls{ris}, \gls{bd}-\gls{ris} significantly improves the dynamic range of all channel singular values, the trade-off in manipulating them, and thus the channel power gain and achievable rate.
	Those observations become more pronounced when \gls{mimo} and \gls{ris} dimensions increase.
	...
	Of particular interest, \gls{bd}-\gls{ris} is shown to activate multi-stream transmission at lower transmit power than \gls{d}-\gls{ris}, hence enhancing the \gls{mimo} multiplexing gain at low \gls{snr} thanks to its higher flexibility of shaping the distribution of channel singular values.
...
\begin{IEEEkeywords}
	Reconfigurable intelligent surface, channel shaping, rate maximization, manifold optimization.
\end{IEEEkeywords}
...
% 		% Those assumptions simplify the problem but limit the generality of the results.
	...
	\begin{subsection}{Contributions}
		...
		% % systems
		% interpret the channel shaping potential of \gls{bd}-\gls{ris} in \gls{mimo} as channel rearrangement and space alignment.
		...
		We highlight that the off-diagonal entries of \gls{bd}-\gls{ris} scattering matrix exploits spatial diversity more effectively than \gls{d}-\gls{ris}.
...
\begin{section}{\glsfmtshort{bd}-\glsfmtshort{ris} Model}
	...
	\begin{itemize}
		...
		In \gls{siso}, \gls{d}-\gls{ris} with perfect phase matching can provide a maximum indirect channel gain $g_\mathrm{D}=\sum_{n=1}^{N_\mathrm{S}} \lvert h_{\mathrm{B},n} \rvert \lvert h_{\mathrm{F},n} \rvert$,
...
\begin{section}{Group-Connected \glsfmtshort{bd}-\glsfmtshort{ris} Design Frameworks}
...
\begin{section}{Simulation Results}\label{sc:simulation}
	...
	\begin{subsection}{Power Gain and Achievable Rate Maximization}
		...
		An interesting observation is that the rate gap between the optimal \gls{ao} approach \eqref{op:rate_passive} -- \eqref{eq:precoder_eigenmode} and the shaping-inspired solution \eqref{eq:scattering_power}, \eqref{eq:precoder_eigenmode} narrows at a larger group size and completely vanishes for a fully-connected \gls{bd}-\gls{ris}.
		This implies that joint \gls{ris}-transceiver designs can be decoupled with marginal performance loss by first shaping the wireless channel and then optimizing the precoder and combiner.
		...
		On the other hand, for $N_\mathrm{T}=N_\mathrm{R}=4$ at $P = \qty{20}{dB}$, the ratio evaluates to \num{1.03}, \num{1.08}, and \num{1.13} for $N_\mathrm{S}=16$, \num{64}, and \num{256}, respectively.
		This phenomenon aligns with the power gain results in Fig. \ref{fg:power_sx} and highlight the shaping benefits of \gls{bd}-\gls{ris} in large-scale \gls{mimo} systems.
