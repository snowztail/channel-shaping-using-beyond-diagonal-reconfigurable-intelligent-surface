...
\begin{section}{Channel Singular Value Redistribution}
	...
	\begin{subsection}{Some Analytical Bounds}
		...
		\begin{proposition}[\gls{los} forward\footnote{A similar result holds for \gls{los} backward channel.} channel]
			...
			\begin{equation}
				\sigma_1(\mathbf{T}) \ge {\color{orange}\sigma_2(\mathbf{H})} \ge \sigma_2(\mathbf{T}) \ge \ldots \ge \sigma_{N-1}(\mathbf{T}) \ge {\color{orange}\sigma_N(\mathbf{H})} \ge \sigma_N(\mathbf{T}),
		...
		We will later show that for a finite $N_\mathrm{S}$, using a larger group size can better approach those bounds.
		...
		\begin{proposition}[fully-connected \gls{ris} without direct link]
			...
			% \begin{equation}
			% 	\prod_{k \in {K}} \sigma_k(\mathbf{H}) \leq \prod_{i \in {I}} \sigma_i(\mathbf{B}) \prod_{j \in {J}} \sigma_j(\mathbf{F}),
			% \end{equation}
