...
\begin{section}{Channel Singular Value Redistribution}
	...
	\begin{subsection}{Pareto Frontier Characterization}
		...
		% For group $g$, the objective \eqref{ob:pareto} is smooth in $\mathbf{\Theta}_g$ and the feasible domain \eqref{co:pareto.unitary} corresponds to the Stiefel manifold.
		...
		% Next, we zoom out to solve general smooth optimization problems of asymmetric \gls{bd}-\gls{ris} with arbitrary group size.
		...
		Next, we zoom out to general smooth maximization problems of asymmetric \gls{bd}-\gls{ris} with arbitrary group size.
		...
		\end{equation}
		% In the Stiefel manifold, the geodesic emanating from $\mathbf{\Theta}_g^{(r)}$ with velocity $\mathbf{D}_g^{(r)}$ and step size $\mu_g$ is described compactly by the exponential map \cite{Edelman1998}
		% In the Stiefel manifold, the geodesic emanating from $\mathbf{\Theta}_g^{(r)}$ with velocity $\mathbf{D}_g^{(r)}$ and step size $\mu_g^{(r)}$ is described compactly by the exponential map \cite{Edelman1998}
		...
		\end{equation}
		% \begin{gather}
		% 	\mathbf{T}_g^{(r)}(\mu) = \exp(\mu \mathbf{D}_g^{(r)}),\label{eq:exponential_map}\\
		% 	\mathbf{G}_g^{(r)}(\mu) = \mathbf{T}_g^{(r)}(\mu) \mathbf{\Theta}_g^{(r)}.\label{eq:geodesic}
		% \end{gather}
		...
		\end{equation}
		% Algorithm~\ref{ag:rcg} summarizes the proposed block-wise geodesic \gls{rcg} update for asymmetric \gls{bd}-\gls{ris} with arbitrary group size.
		Algorithm~\ref{ag:rcg} summarizes the proposed block-wise geodesic \gls{rcg} method for general smooth maximization problems of asymmetric \gls{bd}-\gls{ris}.
		...
		\begin{remark}
			...
			% The advantage of the proposed block-wise \gls{rcg} algorithm over universal manifold optimization \cite{Absil2009,Pan2022d} is trifold:
			\begin{enumerate}
				\item No retraction thanks to rotational update;
		...
		\begin{algorithm}[!t]
			...
			\begin{algorithmic}[1]
				...
				\Repeat
					\For {$g \gets 1$ to $G$}
						\State $\nabla_{\mathrm{R},g}^{(r)} \gets$ \eqref{eq:gradient_euclidean}, \eqref{eq:gradient_riemannian} \label{ln:gradient_euclidean}
						\State $\mathbf{D}_g^{(r)} \gets$ \eqref{eq:parameter_cg}, \eqref{eq:direction_cg}
		...
		% 			\For {$g \gets 1$ to $G$}
		% 				\State $\nabla_{\mathrm{R},g}^{(r)} \gets$ \eqref{eq:gradient_euclidean}, \eqref{eq:gradient_riemannian} \label{ln:gradient_euclidean}
		% 				\State $\gamma_g^{(r)} \gets \begin{cases} 0, & \text{if } r = 1\\ \eqref{eq:parameter_cg}, & \text{otherwise} \end{cases}$
		% 				% \If {$r = 1$}
		% 				% 	\State $\gamma_g^{(r)} \gets 0$
		% 				% \Else
		% 				% 	\State $\gamma_g^{(r)} \gets$ \eqref{eq:parameter_cg}
		% 				% \EndIf
		% 				% \State $\mathbf{D}_g^{(r)} \gets$ \eqref{eq:direction_cg}
		% 				% \If {$\Re\{\mathrm{tr}({\mathbf{D}_g^{(r)}}^\mathsf{H} \nabla_{\mathrm{R},g}^{(r)})\} < 0$}
		% 				% 	\State $\mathbf{D}_g^{(r)} \gets \nabla_{\mathrm{R},g}^{(r)}$
		% 				% \EndIf
