...
\begin{abstract}
	...
	An efficient and universal \gls{bd}-\gls{ris} design framework is proposed over group-wise \emph{geodesic} \gls{rcg} in the Stiefel manifold, which performs multiplicative (instead of add-then-project) updates along the exponential map for faster convergence.
	...
	% Many analytical singular value bounds are also derived for rank-deficient channels and fully-connected (a.k.a. unitary) \gls{ris}.
	For the singular value shaping problem, we characterize their Pareto frontiers by geodesic \gls{rcg} and derive analytical bounds for rank-deficient channels and fully-connected (a.k.a. unitary) \gls{ris}.
	...
	For the power gain maximization problem, we provide a closed-form iterative solution by group-wise \gls{svd} that features even faster convergence.
	...
	Simulation results indicate that the rate deficit from the latter diminishes as the \gls{bd}-\gls{ris} evolves towards unitary, while its shaping advantages over diagonal \gls{ris} increase with \gls{mimo} dimensions and decrease with spatial correlation.
...
\begin{section}{Introduction}
	...
	% a second-order array gain and a fourth-order power scaling law has been reported for \gls{ris} elements \cite{Zhao2022} --- both achieves squared improvements than transceiver antennas.
	% since the coherent scattering can be viewed as equal-gain combining and transmission.
	...
	% which has been investigated \cite{Karasik2020,Basar2020,Zhao2022a} and prototyped \cite{Tang2019a,Dai2020a}.



	% which could be incorporated into the transmitter and receiver for \emph{beamforming}, employed as a free-rider information source for \emph{modulation}, or simply placed in space as a standalone device for \emph{channel shaping}.
	...
	Despite the great potentials and fruitful outcomes, one critical unanswered question is the {channel shaping} capability: \emph{To what extent can a passive \gls{ris} reshape the wireless channel?}
	...
	% This allows the \gls{ris} to control the phase of the scattered wave, but not its amplitude.
	...
	It can thus control both the amplitude and phase of the scattered wave, generalizing the scattering matrix to block-diagonal with unitary blocks.
	...
	% \gls{mimo} channel power maximization problem has been considered in \cite{Nerini2023,Nerini2023b}
	Received power maximization problem is extended to \gls{mimo} for continuous-valued \cite{Nerini2023} and discrete-valued \cite{Nerini2023b} scattering matrices, but it only considered single-stream transmission with given precoder and combiner (which is equivalent to \gls{siso}).
	...
	Although the results are insightful, it remains unclear whether and why the advantage of \gls{bd}-\gls{ris} over diagonal \gls{ris} depends on \gls{mimo} dimensions.
	% , and there lacks a deep understanding of
	...
	Channel shaping is different from passive beamforming because \gls{ris} is exploited as a stand-alone device to modify the inherent properties of the wireless channel.
