...
\begin{section}{Simulation Results}
	...
	\begin{subsection}{Channel Singular Values Redistribution}
		\begin{figure}[!t]
			\centering
			\subfloat[$2 \times 32 \times 2$ (no direct link)\label{fg:pc_singular_pareto_32_nd}]{
				\resizebox{0.48\columnwidth}{!}{
					\input{assets/simulation/pc_singular_pareto_32_nd.tex}
				}
			}
			\subfloat[$2 \times 32 \times 2$\label{fg:pc_singular_pareto_32}]{
				\resizebox{0.48\columnwidth}{!}{
					\input{assets/simulation/pc_singular_pareto_32.tex}
				}
			}
			\\
			\subfloat[$2 \times 64 \times 2$\label{fg:pc_singular_pareto_64}]{
				\resizebox{0.48\columnwidth}{!}{
					\input{assets/simulation/pc_singular_pareto_64.tex}
				}
			}
			\subfloat[$2 \times 128 \times 2$\label{fg:pc_singular_pareto_128}]{
				\resizebox{0.48\columnwidth}{!}{
					\input{assets/simulation/pc_singular_pareto_128.tex}
				}
			}
		...
		\begin{figure}[!t]
			\centering
			\subfloat[$4 \times 32 \times 4$\label{fg:pc_singular_bound_32}]{
				\resizebox{0.48\columnwidth}{!}{
					\input{assets/simulation/pc_singular_bound_32.tex}
				}
			}
			\subfloat[$4 \times 64 \times 4$\label{fg:pc_singular_bound_64}]{
				\resizebox{0.48\columnwidth}{!}{
					\input{assets/simulation/pc_singular_bound_64.tex}
				}
			}
			\caption{
				Channel singular values with \gls{los} forward channel ($\kappa_\mathrm{F} = 0$): analytical bounds (green lines) and optimized results (blue and red bars).
				`D' means diagonal \gls{ris} and `BD' means fully-connected \gls{bd}-\gls{ris}.
				% $\kappa_\mathrm{D}=\kappa_\mathrm{B}=0$, $\kappa_\mathrm{F}=\infty$.
			}
			% \caption{Singular values of equivalent channel with \gls{los} forward channel: analytical bounds (green lines) and optimized results (blue and red bars).}
		...
		% The Pareto frontier of channel singular values reshaped by \gls{bd}-\gls{ris} is shown in Fig. \ref{fg:pc_singular_pareto}.
		% The Pareto frontier of channel singular values reshaped by \gls{bd}-\gls{ris} is shown in Fig. \ref{fg:pc_singular_pareto}.
		% Clearly, a larger group size provides
		% and evolving trend of channel singular values are shown in Fig.~\ref{sm:pc_singular_pareto} and \ref{sm:pc_singular_bound}.
...
% \begin{appendix}
	...
	% \begin{subsection}{Proof of Proposition \ref{pp:los}}\label{ap:los}
	% 	Let $\mathbf{H}_\mathrm{F} = \sigma_\mathrm{F} \mathbf{u}_\mathrm{F} \mathbf{v}_\mathrm{F}^\mathsf{H}$ be the compact \gls{svd} of the forward channel.
	% 	The channel Gram matrix can be written as Hermitian-plus-rank-1:
	% 	\begin{equation}
	% 		\mathbf{G} \triangleq \mathbf{H} \mathbf{H}^\mathsf{H} = \mathbf{Y} + \mathbf{z} \mathbf{z}^\mathsf{H},
