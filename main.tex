...
\begin{abstract}
	...
	Theoretical analysis and numerical evaluation reveal that the shaping advantage of \gls{bd}-\gls{ris} increases with group size, stemming from stronger subchannel rearrangement and subspace alignment capabilities.
\end{abstract}
...
\begin{section}{Introduction}
	...
	Today we are witnessing a paradigm shift from connectivity to intelligence, where the wireless environment is no longer a chaotic medium but a conscious agent that respects customization.
	% serves on demand.
	...
	In contrast, \gls{bd}-\gls{ris} can provide a higher channel power but the results are limited to \gls{siso}\footnote{In terms of channel shaping, single-stream \gls{mimo} with given precoder and combiner \cite{Nerini2023} is equivalent to \gls{siso}.}. \cite{Nerini2023} and \gls{miso} \cite{Santamaria2023}.
	...
	The proposed method features lower complexity and faster convergence than general manifold optimization \cite{Absil2009,Pan2022d}, which is used to solve the Pareto singular value problem.
	...
	Fourth, extensive simulations reveal that the advantage of \gls{bd}-\gls{ris} over diagonal \gls{ris} increases with group size and \gls{mimo} dimensions.
	In terms of channel power, fully-connected \gls{bd}-\gls{ris} boosts up to 62\%, 312\%, 537\% in $1 \times 1$, $4 \times 4$, $16 \times 16$ configuration under Rayleigh fading.
