...
\begin{section}{Channel Singular Values Redistribution}
	...
	\begin{subsection}{Some Analytical Bounds}\label{sc:bounds}
		...
		Corollaries \ref{co:sv_smallest} and \ref{co:sv_product_smallest} are less informative when $\bar{N} \ne N$ (i.e., unequal number of transmit, scatter, and receive antennas) as the lower bounds would be zero.
		...
		% and $f_{\lambda_{\mathrm{F},n}}$ are their corresponding probability density functions.

		% the singular values of a rank-1 matrix are invariant under unitary transformations.

		% unless both $\mathbf{H}_\mathrm{B}$ and $\mathbf{H}_\mathrm{F}$ are rank-1.
		% optimal passive \gls{ris} design should align the subspaces of $\mathbf{H}_\mathrm{B}$ and $\mathbf{H}_\mathrm{F}$ by rotation and pair the subchannels from strongest to weakest.
		% When both backward and forward channels are rank-1
		% When the backward and forward channels are in Rayleigh fading,
		% we can derive an explicit upper bound on the channel power gain when the backward and forward channels are Rayleigh fading.
		...
		Tighter bounds than Proposition \ref{pp:rank_deficient} are generally inapplicable when a \gls{mimo} direct link is present or the \gls{bd}-\gls{ris} is not unitary.
		% Proposition \ref{pp:fully_connected} only applies to unitary \gls{ris} without direct link, since t
		This is because there exists a trade-off between the direct-indirect and backward-forward subspace alignments.
		In such case, a balance could be struck by optimization approaches from a singular value perspective (discussed in Section~\ref{sc:pareto_frontier}) or a power gain perspective (will be discussed in Section~\ref{sc:rate}).
	\end{subsection}
\end{section}
...
\begin{section}{Simulation Results}\label{sc:simulation}
	...
	\begin{subsection}{Channel Singular Values Redistribution}
		...
		\begin{subsubsection}{Analytical Bounds and Numerical Results}
			...
			\end{figure}
			Fig. \ref{fg:singular_bound} illustrates the analytical singular value bounds in Proposition \ref{pp:rank_deficient} and the numerical results obtained by solving problem \eqref{op:pareto} with $\rho_n = \pm 1$ and $\rho_{\bar{N}} = 0$, $\forall \bar{N} \ne n$.
			...
			Fig. \ref{fg:power_bond} compares the analytical channel power bound in Corollary \ref{co:sum_power} with $k=\bar{N}$, $p=2$ and the numerical results obtained by solving problem \eqref{op:power_passive} when the direct link is absent.
