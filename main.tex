...
\begin{section}{Channel Singular Value Redistribution}
	...
	\begin{subsection}{Some Analytical Bounds}
		...
		\end{equation}
		\begin{equation}
			U_r^n = \Bigl\{(I, J, K) \mid \sum_{i \in I} i + \sum_{j \in J} j = \sum_{k \in K} k + r(r+1)/2\Bigr\}.
		\end{equation}
		...
		\begin{itemize}
			...
			\end{equation}
			when $k = N$ and $p = 2$, it suggests the channel power is upper bounded by the sum of element-wise product of backward and forward subchannel power.
		\end{itemize}
		...
		\begin{remark}
			...
			\begin{itemize}
				...
				\item subchannel rearrangement: $\mathbf{X} = \mathbf{I}$ in \eqref{eq:channel_equivalent_fc} pairs the components of $\mathbf{H}_\mathrm{B}$ and $\mathbf{H}_\mathrm{F}$ by strength, which is optimal by rearrangement inequality.
			\end{itemize}
		\end{remark}

		Tight bounds are generally unavailable when the direct link exists, as the \gls{ris} needs to balance the backward-forward (multiplicative) and direct-indirect (additive) alignments.
