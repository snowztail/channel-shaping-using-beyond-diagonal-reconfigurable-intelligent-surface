...
\begin{abstract}
	This paper investigates how significantly a passive \gls{ris} can redistribute the singular values of a \gls{mimo} point-to-point channel.
	% the limit to which
	We depart from the widely-adopted diagonal phase shift model to a \gls{bd}-\gls{ris} architecture featuring in-group connections between elements.
	% This powerful model allows cooperative wave scattering with both amplitude and phase control, and we uniquely attribute the shaping advantage to subchannel rearrangement and subspace alignment.
	This powerful model enables cooperative wave scattering with both amplitude and phase control, and we provide unique shaping insights in terms of subchannel rearrangement and subspace alignment.
	% uniquely explain the extra design freedom from off-diagonal entries as subchannel rearrangement and subspace alignment.
	% while the shaping advantage from off-diagonal entries is uniquely attributed to subchannel rearrangement and subspace alignment.
	...
	% We uniquely attribute the shaping advantage from off-diagonal entries to subchannel rearrangement and subspace alignment.
	...
	As a side product, we tackle \gls{ris}-aided \gls{mimo} achievable rate maximization by a local-optimal \gls{ao} approach and a low-complexity shaping-based approach.
	Simulation results highlight that \gls{bd}-\gls{ris} provides a wider dynamic range and better trade-off on channel singular values, while its power and rate gains over diagonal \gls{ris} increase with \gls{mimo} dimensions.
...
\begin{section}{Introduction}
	...
	This creates a free-ride message stream (similar to index modulation \cite{Basar2017}) with dual benefits: integrating with the legacy transmitter for enhanced channel capacity \cite{Karasik2020,Basar2020,Ye2022} or serving as a dedicated source for passive uplink communication \cite{Liang2020,Zhao2022a,Yang2024}.
	...
	This provides a ubiquitous wave scattering benchmark for different wireless applications and greatly simplifies the \gls{ris}-transceiver design.
	...
	% \begin{enumerate}
	% 	\item What is the entire achievable singular value region of a \gls{mimo} channel with \gls{ris}?
	% \end{enumerate}
	...
	This ideally translates to a scattering matrix with unit-magnitude entries on the main diagonal and zeros elsewhere, applying merely a phase shift to the signal.
	...
	Such a powerful model can be realized at reduced cost using tree- and forest-connected architectures inspired by graph theory \cite{Nerini2024}.
	...
	Its beamforming superiority has been studied extensively in \gls{siso} and \gls{miso} systems in terms of single-user \gls{snr} maximization \cite{Shen2020a,Nerini2023,Nerini2024,Santamaria2023} and multi-user \gls{wsr} maximization \cite{Fang2023,Zhou2023,Li2023c,Soleymani2024}.
	...
	Similar constraints were also adopted in \cite{Mishra2024}, which introduces transmitter-side \gls{bd}-\gls{ris} to massive \gls{mimo} for improved spectral efficiency.
	Received power maximization problem was studied over continuous-valued \cite{Nerini2023} and discrete-valued \cite{Nerini2023b} block-unitary scattering matrices, but the proposed single-stream transceiver is rate-suboptimal and the problem is equivalent to \gls{siso}.
	...
	Although the results are promising, there lacks a study of \gls{bd}-\gls{ris} in general \gls{mimo} systems and its shaping potential over diagonal \gls{ris} has not been fully understood.
	% remains unclear how the off-diagonal entries contribute to channel shaping and whether the relative gain of \gls{bd}-\gls{ris} over diagonal \gls{ris} scales with \gls{mimo} dimensions.
