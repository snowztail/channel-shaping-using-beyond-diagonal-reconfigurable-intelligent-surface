...
\begin{section}{Introduction}
	...
	\begin{subsection}{Related Works}
		...
		The idea was soon generalized to \gls{bd}-\gls{ris} with group-connected architecture \cite{Shen2020a}, where adjacent elements within the same group are connected via passive reconfigurable components\footnote{Those components can be either symmetric (e.g., capacitors and inductors) or asymmetric (e.g., ring hybrids and branch-line hybrids) \cite{Ahn2006}, resulting in symmetric and asymmetric scattering matrices, respectively.\label{fn:symmetric}}.
...
\begin{section}{\glsfmtshort{bd}-\glsfmtshort{ris} Model}
	...
	% We conclude from \eqref{eq:channel_equivalent_svd} that the scattering matrix of \gls{bd}-\gls{ris} provide two channel shaping potentials:
	...
	\begin{itemize}
		...
		% When it comes to \gls{mimo}, each \gls{d}-\gls{ris} element can only apply a scalar phase shift to the corresponding indirect channel matrix of size $N_\mathrm{R} \times N_\mathrm{T}$ defined in \eqref{eq:channel_equivalent}.
	...
	% \begin{remark}
	% 	From \eqref{eq:scattering_fc} and \eqref{eq:channel_equivalent_fc} in the proof of Proposition \ref{pp:direct_negligible}, we notice that \eqref{iq:sv_bound_fc}--\eqref{iq:sv_bound_fc_power} are simultaneously tight when
...
\begin{section}{Analytical Bounds on Singular Value Shaping}
	...
	% Example~\ref{eg:d_vs_bd} shows that physically grouping some scattering elements produces a ``cooperation effect'' that significantly enhances the dynamic range of channel singular values.
	...
	% We also provide some analytical bounds related to channel singular values, which help us to explore the limits of channel shaping using passive \gls{ris}.
		...
		We emphasize that
		% the inequalities in \eqref{iq:sv_bound_rank_deficient} and \eqref{iq:sv_bound_los}
		...
		% Proposition \ref{pp:rank_deficient} evaluates the ultimate channel shaping limits of passive \gls{ris} and provides a benchmark for $N_\mathrm{S}$ selection in low-multipath deployment scenarios.
		...
		% Besides, \eqref{iq:sv_bound_fc} helps to induce lower bounds
		...
		\begin{corollary}[Product of some largest or smallest singular values\label{co:sv_product}]
			% \footnote{Corollaries \ref{co:sv_smallest} and \ref{co:sv_product_smallest} are less informative when $\bar{N} \ne N$ (i.e., $N_\mathrm{T} = N_\mathrm{S} = N_\mathrm{R}$ being false) as the lower bounds would coincide at zero.}
			% For any $k \in [\bar{N}]$,
		...
		\begin{corollary}[Individual singular value\label{co:sv_nth}]
			...
			% \begin{subequations}
			% 	\begin{align}
			% 		\mathbf{\Theta}_{\textnormal{sv-}n\textnormal{-max}}^\textnormal{MIMO-ND} & = \mathbf{V}_\mathrm{B} \mathbf{P} \mathbf{\Theta}_\mathrm{D} \mathbf{U}_\mathrm{F}^\mathsf{H}, \label{eq:sv_nth_max} \\
			% 		\mathbf{\Theta}_{\textnormal{sv-}n\textnormal{-min}}^\textnormal{MIMO-ND} & = \mathbf{V}_\mathrm{B} \mathbf{Q} \mathbf{\Theta}_\mathrm{D} \mathbf{U}_\mathrm{F}^\mathsf{H}, \label{eq:sv_nth_min}
			% 	\end{align}
			% \end{subequations}
			\begin{subequations}
				\begin{align}
					\mathbf{\Theta}_{\textnormal{sv-}n\textnormal{-max}}^\textnormal{MIMO-ND} & = \mathbf{V}_\mathrm{B} \mathbf{L} \mathbf{P} \mathbf{R} \mathbf{U}_\mathrm{F}^\mathsf{H}, \label{eq:sv_nth_max} \\
					\mathbf{\Theta}_{\textnormal{sv-}n\textnormal{-min}}^\textnormal{MIMO-ND} & = \mathbf{V}_\mathrm{B} \mathbf{L} \mathbf{Q} \mathbf{R} \mathbf{U}_\mathrm{F}^\mathsf{H}, \label{eq:sv_nth_min}
				\end{align}
			\end{subequations}
			where $\mathbf{V}_\mathrm{B}, \mathbf{U}_\mathrm{F} \in \mathbb{U}^{N_\mathrm{S} \times N_\mathrm{S}}$ are the right and left singular matrices of $\mathbf{H}_\mathrm{B}$ and $\mathbf{H}_\mathrm{F}$, respectively, $\mathbf{L}, \mathbf{R}$ are arbitrary block-diagonal unitary matrices with size of block $n \in [N_\mathrm{S}]$ equal to the multiplicity of $\sigma_n(\mathbf{H}_\mathrm{B})$ and $\sigma_n(\mathbf{H}_\mathrm{B})$, respectively,
			% $\mathbf{\Theta}_\mathrm{D}$ is arbitrary diagonal matrix with unit-magnitude diagonal entries,
			and $\mathbf{P}$ and $\mathbf{Q}$ are arbitrary doubly stochastic matrices of dimension $N_\mathrm{S}$ satisfying:
			\begin{itemize}
				\item The $(i, j)$-th entries sum up to $1$, where
				\begin{equation*}
					(i, j) =
					\begin{cases}
						\underset{i+j=n+1}{\arg\min} \sigma_i(\mathbf{H}_\mathrm{B}) \sigma_j(\mathbf{H}_\mathrm{F}) \triangleq \mathcal{S}_n(\mathbf{P}) & \text{for } \mathbf{P}, \\
						\underset{i+j=n+\bar{N}}{\arg\max} \sigma_i(\mathbf{H}_\mathrm{B}) \sigma_j(\mathbf{H}_\mathrm{F}) \triangleq \mathcal{S}_n(\mathbf{Q}) & \text{for } \mathbf{Q};
					\end{cases}
				\end{equation*}
				\item The submatrix with $i$-th rows and $j$-th columns deleted, $(i,j) \in \mathcal{S}_n$, denoted as $\mathbf{Y}$, is arbitrary doubly stochastic matrix of dimension $N_\mathrm{S}-\card(\mathcal{S}_n)$ satisfying
				\begin{align*}
					...
					\sigma_{n+1}(\hat{\mathbf{\Sigma}}_{\mathrm{B}} \mathbf{Y} \hat{\mathbf{\Sigma}}_{\mathrm{F}}) & \le \max_{i+j=n+\bar{N}} \sigma_i(\mathbf{H}_\mathrm{B}) \sigma_j(\mathbf{H}_\mathrm{F}) && \text{for } \mathbf{Q},
				\end{align*}
				where $\hat{\mathbf{\Sigma}}_{\mathrm{B}}$ and $\hat{\mathbf{\Sigma}}_{\mathrm{F}}$ are diagonal singular value matrices of $\mathbf{H}_\mathrm{B}$ and $\mathbf{H}_\mathrm{F}$ with the $i$-th rows and $j$-th columns deleted, $(i,j) \in \mathcal{S}_n$.
			\end{itemize}

			% where $\mathbf{P}, \mathbf{Q}$ can be any unitary matrix whose $(i, j)$-th entry is of unit-magnitude, and $(i, j) = \arg \min_{i+j=n+1} \sigma_i(\mathbf{H}_\mathrm{B}) \sigma_j(\mathbf{H}_\mathrm{F})$ for $\mathbf{P}$ and $(i, j) = \arg \max_{i+j=n+\bar{N}} \sigma_i(\mathbf{H}_\mathrm{B}) \sigma_j(\mathbf{H}_\mathrm{F})$ for $\mathbf{Q}$.
			% where $\mathbf{P}, \mathbf{Q}$ can be any unitary matrix whose $(i, j)$-th entry is of unit-magnitude, and $(i, j) = \arg \min_{i+j=n+1} \sigma_i(\mathbf{H}_\mathrm{B}) \sigma_j(\mathbf{H}_\mathrm{F})$ for $\mathbf{P}$ and $(i, j) = \arg \max_{i+j=n+\bar{N}} \sigma_i(\mathbf{H}_\mathrm{B}) \sigma_j(\mathbf{H}_\mathrm{F})$ for $\mathbf{Q}$.
			% that reorders the singular values of $\mathbf{H}_\mathrm{B}$ and $\mathbf{H}_\mathrm{F}$.
		\end{corollary}
		...
		\begin{proof}
			...
			To attain the upper bound, the \gls{bd}-\gls{ris} needs to maximize the minimum of the first $n$ singular values of $\mathbf{H}$.
			It follows from \eqref{eq:sv_nth_max} that
			\begin{equation*}
				\begin{split}
					\sv(\mathbf{H})
					& = \sv(\mathbf{H}_\mathrm{B} \mathbf{V}_\mathrm{B} \mathbf{P} \mathbf{\Theta}_\mathrm{D} \mathbf{U}_\mathrm{F}^\mathsf{H} \mathbf{H}_\mathrm{F}) \\
					& = \sv(\mathbf{U}_\mathrm{B} \mathbf{\Sigma}_\mathrm{B} \mathbf{P} \mathbf{\Theta}_\mathrm{D} \mathbf{\Sigma}_\mathrm{F} \mathbf{U}_\mathrm{F}^\mathsf{H}) \\
					& = \sv(\mathbf{U}_\mathrm{B} \mathbf{\Sigma}_\mathrm{B} \mathbf{P} \mathbf{\Sigma}_\mathrm{F} \mathbf{\Theta}_\mathrm{D} \mathbf{U}_\mathrm{F}^\mathsf{H}) \\
					& = \sv(\mathbf{\Sigma}_\mathrm{B} \mathbf{P} \mathbf{\Sigma}_\mathrm{F}) \\
				\end{split}
			\end{equation*}
			where $\mathbf{H}_\mathrm{B} = \mathbf{U}_\mathrm{B} \mathbf{\Sigma}_\mathrm{B} \mathbf{V}_\mathrm{B}^\mathsf{H}$ and $\mathbf{H}_\mathrm{F} = \mathbf{U}_\mathrm{F} \mathbf{\Sigma}_\mathrm{F} \mathbf{V}_\mathrm{F}^\mathsf{H}$.

			The doubly stochastic matrix $\mathbf{P}$ should align the modes of $\mathbf{H}_\mathrm{B}$ and $\mathbf{H}_\mathrm{F}$ such that the first $n$ modes of $\mathbf{H}$ are maximized.

			One feasible solution is to align and order the first $n$ modes of $\mathbf{H}_\mathrm{B}$ and $\mathbf{H}_\mathrm{F}$, resulting in $\sigma_n(\mathbf{H}) = \min_{i+j=n+1} \sigma_i(\mathbf{H}_\mathrm{B}) \sigma_j(\mathbf{H}_\mathrm{F})$.
			If such $(i,j)$ pairs are not unique, then any mixture of them would produce the same result.
			% We emphasize that the solution is not unique and mixture of such $(i,j)$ pairs would produce the same result.
			% It is worth mentioning that the solution is not unique and any mixture of such $(i,j)$ pairs would produce the same result.
			However, since we only care about the $n$-th singular value, the first $n-1$ singular values can be arbitrarily reordered.
			% This is achieved by aligning and reordering the first $n$ modes of $\mathbf{H}_\mathrm{B}$ and $\mathbf{H}_\mathrm{F}$.

			proper matching of the first $n$ modes of $\mathbf{H}_\mathrm{B}$ to the first $n$ modes of $\mathbf{H}_\mathrm{F}$.
		...
		Now we can see the alignment by $V_B$ $U_F$ and ordering by $P$ $Q$.

		We highlight the solution (15) is not unique.

		In particular, the extreme singular values can be manipulated up to
		\begin{subequations}
			\label{iq:sv_extreme}
			\begin{gather}
			...
			\end{gather}
		\end{subequations}
		where the arguments of $\max$ and $\min$ are essentially $\sigma_1(\mathbf{H}_\mathrm{B}) \sigma_{\bar{N}}(\mathbf{H}_\mathrm{F}), \sigma_2(\mathbf{H}_\mathrm{B}) \sigma_{\bar{N}-1}(\mathbf{H}_\mathrm{F}), \ldots, \sigma_{\bar{N}}(\mathbf{H}_\mathrm{B}) \sigma_1(\mathbf{H}_\mathrm{F})$.
		We notice that the right side inequalities in \eqref{iq:sv_largest} and \eqref{iq:sv_smallest} are special cases of \eqref{iq:sv_product_largest} and \eqref{iq:sv_product_smallest} when $k=1$, respectively.
		It is evident from \eqref{iq:sv_nth} that to enlarge the $n$-th singular value of $\mathbf{H}$, one should

		enumerate all matching pairs $(i, j) \in [\bar{N}]^2 \mid i+j=n+1$, find the one with the minimum product singular value

		match the singular

		align the $i$-th singular value of $\mathbf{H}_\mathrm{B}$ with the $j$-th singular value of $\mathbf{H}_\mathrm{F}$ such that $i+j=n+\bar{N}$.

		% The lower bound on the smallest singular value reduces to 0 when $\bar{N} \ne N$ (i.e., $N_\mathrm{T} = N_\mathrm{S} = N_\mathrm{R}$ being false).
		% in \eqref{iq:sv_smallest} is less informative when $\bar{N} \ne N$ (i.e., $N_\mathrm{T} = N_\mathrm{S} = N_\mathrm{R}$ being false) as the lower bounds would coincide at zero.
		% in \eqref{iq:sv_product_smallest} and \eqref{iq:sv_smallest} reduce to 0 when $\bar{N} \ne N$ (i.e., $N_\mathrm{T} = N_\mathrm{S} = N_\mathrm{R}$ being false).
		...
		\begin{corollary}[Channel power gain\label{co:sum_power}]
			...
			\begin{subequations}
				\begin{align}
					\mathbf{\Theta}_\textnormal{P-max}^\textnormal{MIMO-ND} & = \mathbf{V}_\mathrm{B} \mathbf{\Theta}_\mathrm{D} \mathbf{U}_\mathrm{F}^\mathsf{H}, \\
					\mathbf{\Theta}_\textnormal{P-min}^\textnormal{MIMO-ND} & = \mathbf{V}_\mathrm{B} \mathbf{J} \mathbf{\Theta}_\mathrm{D} \mathbf{U}_\mathrm{F}^\mathsf{H},
				\end{align}
			\end{subequations}
			% \begin{equation}
			% 	\mathbf{\Theta}_\textnormal{P-max}^\textnormal{MIMO-ND} = \mathbf{V}_\mathrm{B} \mathbf{U}_\mathrm{F}^\mathsf{H}, \quad \mathbf{\Theta}_\textnormal{P-min}^\textnormal{MIMO-ND} = \mathbf{V}_\mathrm{B} \mathbf{J} \mathbf{U}_\mathrm{F}^\mathsf{H},
			% \end{equation}
		...
		% ! Talk about when is the assumption hold.
		% We highlight that the bounds in Corollaries \ref{co:sv_bound_horn} -- \ref{co:sum_power} are asymptotically tight as $N_\mathrm{S}$ goes to infinity.
		...
		% % TODO enumerate all bounds for 2x2x2
		% \begin{example}[Individual (no, all) singular value bounds on $2 \times 2 \times 2$ shaping]\label{eg:bounds}
		% 	Consider a $2 \times 2 \times 2$ setup with
		% 	% $\sv(\mathbf{H}_\mathrm{B}) = [3, 2]^\mathsf{T}$ and $\sv(\mathbf{H}_\mathrm{B}) = [4, 1]^\mathsf{T}$.
		% 	\begin{equation*}
		% 		\mathbf{H}_\mathrm{B} =
		% 			\begin{bmatrix}
		...
		% 			\end{bmatrix}.
		% 	\end{equation*}
		% 	It is obvious that any \gls{d}-\gls{ris} can only attain $\sv(\mathbf{H}) = [8, 3]^\mathsf{T}$ while a \gls{bd}-\gls{ris} $\mathbf{\Theta} = \begin{bsmallmatrix} 0 & 1 \\ 1 & 0 \end{bsmallmatrix}$ can match the stronger branches by unitary transformation to achieve $\sv(\mathbf{H}) = [12, 2]^\mathsf{T}$.
		% 	The bounds on individual singular values are $8 \le \sigma_1(\mathbf{H}) \le 12$ and $2 \le \sigma_2(\mathbf{H}) \le 3$.
		% 	However, the upper/lower bounds are not simultaneously achievable due to the trade-off between two modes.
		% 	% and the singular value region is a subset of the rectangle due to the trade-off between the alignment of two modes.
		% 	% but there exists a trade-off in between due to mode alignment.
		...
		% , widening the singular values to $8 \le \sigma_1(\mathbf{H}) \le 12$ and $2 \le \sigma_2(\mathbf{H}) \le 3$.
		That is, fully-connected \gls{bd}-\gls{ris} applies right unitary transformation on $\mathbf{H}_\mathrm{B}$ and left unitary transformation on $\mathbf{H}_\mathrm{F}$, widening the singular values to $8 \le \sigma_1(\mathbf{H}) \le 12$ and $2 \le \sigma_2(\mathbf{H}) \le 3$.

		% We thus conclude that a fully-connected \gls{bd}-\gls{ris} can apply right unitary transformation on $\mathbf{H}_\mathrm{B}$ and left unitary transformation on $\mathbf{H}_\mathrm{F}$, which widens channel singular values to $8 \le \sigma_1(\mathbf{H}) \le 12$ and $2 \le \sigma_2(\mathbf{H}) \le 3$ in this example.
		...
		% 		& = \sum_n \int_0^\infty f_{\lambda_n^{\min(N_\mathrm{R},N_\mathrm{S})}}(x_n) d x_n \\
		% 		& \quad \times \int_0^\infty f_{\lambda_n^{\min(N_\mathrm{S},N_\mathrm{T})}}(x_n) d x_n,
		% 	\end{split}
		% 	\label{eq:power_gain_rayleigh}
		% \end{equation}
		...
		% Expression \eqref{eq:power_gain_rayleigh} generalizes the \gls{siso} power gain \cite[Equation 58]{Shen2020a} to \gls{mimo} and unveils

		% , unveiling how the channel power gain scales with the number of transmit and receive antennas and \gls{bd}-\gls{ris} elements under double Rayleigh fading.
		% is a generalization of \gls{siso} power gain \cite[Equation 58]{Shen2020a} to \gls{mimo}, which unveils how the channel power gain scales with the number of transmit and receive antennas and \gls{bd}-\gls{ris} elements under double Rayleigh fading.
		% However, a closed-form expression is non-trivial due to the complexity of distribution of ordered eigenvalues of Wishart matrices.
		% ordered Wishart matrix eigenvalue distribution.
		% provides a benchmark for the channel power gain of \gls{bd}-\gls{ris} in Rayleigh fading channels.
		...
		% When the direct channel is present,
		% Similar results are inapplicable
	...
	% numerical and analytical studies on singular value redistribution by \gls{bd}-\gls{ris} in Sections~\ref{sc:pareto_frontier} and \ref{sc:bounds}, respectively.
	% further studies on channel shaping.

	% Interestingly, connecting two scattering elements via a symmetric component can provide a much larger dynamic range of channel singular values.
	% singular value manipulation range than using two independent elements.
	% that such a large difference comes from connecting both scattering elements using a symmetric component.
	% , where the former needs 2 reconfigurable impedance and the latter needs 3.
	% It is interesting that the huge difference comes purely from connecting both elements.
	% Such a large gain comes from simply connecting both elements
	% , and it can be even larger using an asymmetric component.
	% an even larger gain is expected when an asymmetric reconfigurable component is available.
	% An even larger gain is expected when an asymmetric reconfigurable component is available.
	...
	% 	We showcase the channel shaping capabilities of different \gls{ris} models by a toy example.
	% 	Consider a $2 \times 2 \times 2$ setup where the direct link is blocked.
	% 	The \gls{d}-\gls{ris} is modeled by $\mathbf{\Theta}_\mathrm{D} = \diag(e^{\jmath \theta_1}, e^{\jmath \theta_2})$ while the fully-connected \gls{bd}-\gls{ris} can be modeled by 4 independent angular parameters
	% 	\begin{equation}
	...
	% 		\label{eq:unitary_ris}
	% 	\end{equation}
	% 	It is worth noting that $\phi$ has no impact on the singular value because $\sv(e^{\jmath \phi} \mathbf{A}) = \sv(\mathbf{A})$.
	...
	% 		\label{fg:singular_trend}
	% 	\end{figure}
	% 	Fig.~\ref{fg:singular_trend} compares the achievable singular values of one typical channel realization, where the backward and forward channels are given by
	% 	\begin{equation*}
	% 		\begin{split}
	% 			\mathbf{H}_\mathrm{B} & =
	% 			\begin{bmatrix}
	...
	% 	% , where the former needs 2 reconfigurable impedance and the latter needs 3.
	% 	% It is interesting that the huge difference comes purely from connecting both elements.
	% 	Such a large gain comes from simply connecting both elements, and it can be even larger using an asymmetric component.
	% 	% an even larger gain is expected when an asymmetric reconfigurable component is available.
	% 	% An even larger gain is expected when an asymmetric reconfigurable component is available.
	% 	% and a larger gain is expected when an asymmetric connection is available.
	% 	This example shows that \gls{bd}-\gls{ris} can provide a wider dynamic range of channel singular values and motivates further studies on channel shaping.
...
\begin{section}{\glsfmtshort{bd}-\glsfmtshort{ris} Design Frameworks}
	...
	\begin{subsection}{General (Non-Geodesic) \glsfmtshort{rcg}}
		...
		% the points are updated in the linear embedding spaces by addition \eqref{eq:update_additive} and retraction \eqref{eq:retraction}, instead of on the Stiefel manifold itself.
			...
			\begin{algorithmic}[1]
				...
				\Repeat
					\For {$g \gets 1$ to $G$}
						...
						\While {$f\bigl(\mathbf{G}_g^{(r)}(2\mu)\bigr) - f(\mathbf{\Theta}_g^{(r)}) \ge \mu \cdot \tr(\mathbf{D}_g^{(r)} {\mathbf{D}_g^{(r)\mathsf{H}}}) / 2$} \label{ln:armijo_start}
							\State $\mu \gets 2 \mu$
						\EndWhile
						\While {$f\bigl(\mathbf{G}_g^{(r)}(\mu)\bigr) - f(\mathbf{\Theta}_g^{(r)}) < \mu / 2 \cdot \tr(\mathbf{D}_g^{(r)} {\mathbf{D}_g^{(r)\mathsf{H}}}) / 2$}
							\State $\mu \gets \mu / 2$
...
% 	\label{sc:power_rate}
	...
	\begin{subsection}{Channel Power Gain}\label{sc:power}
		...
		\begin{maxi!}
			...
			\addConstraint{\mathbf{\Theta}_g^\mathsf{H} \mathbf{\Theta}_g=\mathbf{I}, \quad \forall g,}{\label{cs:power_block_unitary}}{}
		\end{maxi!}
		...
		\begin{remark}
			...
			\begin{mini!}
				...
				\addConstraint{\mathbf{\Theta}^\mathsf{H} \mathbf{\Theta}=\mathbf{I},}{\label{cs:power_unitary}}{}
			\end{mini!}
...
\begin{section}{Simulation Results}\label{sc:simulation}
	...
	\begin{subsection}{Channel Singular Values Redistribution}
		...
		\begin{subsubsection}{Analytical Bounds and Numerical Results}
			...
			Interestingly, this relative power gain surges to \qty{270}{\percent} in $N_\mathrm{T}=N_\mathrm{R}=4$ \gls{mimo} as shown in Fig. \subref*{fg:power_bond_txrx4_nd}, which can also be predicted from the expectation analysis \eqref{eq:power_gain_rayleigh}.
			We thus conclude that the power gain of \gls{bd}-\gls{ris} scales with group size and \gls{mimo} dimensions.
		\end{subsubsection}
...
\begin{appendix}
	...
	\begin{subsection}{Proof of Proposition \ref{pp:power}}\label{ap:power}
		...
		\begin{maxi!}
			{\scriptstyle{\mathbf{\Theta}}}{\sum_g 2 \Re\bigl\{ \tr(\mathbf{\Theta}_g^\mathsf{H} \mathbf{M}_g) \bigr\}}{\label{op:power_passive_approx}}{\label{ob:power_passive_approx}}
			\addConstraint{\mathbf{\Theta}_g^\mathsf{H} \mathbf{\Theta}_g=\mathbf{I}, \quad \forall g.}{}{}
		\end{maxi!}
