...
% 		% with blocked direct link and \gls{bd}-\gls{ris}.
% 		% Those assumptions simplify the problem but limit the generality of the results.
		...
		% improve the spectral efficiency of massive \gls{mimo} systems where the direct link is blocked \cite{Mishra2024}.
		massive \gls{mimo} systems that exploits statistical \gls{csi} for improved spectral efficiency \cite{Mishra2024}, which again assumed blocked direct channel and fully-connected \gls{bd}-\gls{ris}.
	...
	\begin{subsection}{Contributions}
		...
		For rank-deficient forward/backward channel, we derive singular value bounds applying to \gls{d}- and \gls{bd}-\gls{ris} with asymptotically large number of elements.
...
\begin{section}{\glsfmtshort{bd}-\glsfmtshort{ris} Model}
	...
	The overall scattering matrix of the \gls{bd}-\gls{ris} is block-diagonal with unitary diagonal blocks\footnote{Following footnote \ref{fn:symmetric}, we do not limit the scattering matrix to be symmetric. One can enforce symmetry over the result by $\mathbf{\Theta} \gets (\mathbf{\Theta} + \mathbf{\Theta}^\mathsf{T})/2$ if necessary.}
	\begin{equation}
	...
	\begin{itemize}
		...
		\item \emph{Space alignment:} It refers to aligning the singular vector spaces of the direct, forward, and backward channels.
...
\begin{section}{\glsfmtshort{bd}-\glsfmtshort{ris} Design Frameworks}
	...
	\end{maxi!}
	where the objective function $f(\mathbf{\Theta})$ can be any function of the \gls{bd}-\gls{ris} scattering matrix, for example, channel singular value (covered in Section \ref{sc:singular_value}), channel power gain (covered in Section \ref{sc:power_rate}), and achievable rate (covered in Section \ref{sc:power_rate}).
	...
	The former solves unconstrained problem \eqref{ob:general} by quasi-Newton methods then projects the solution back to domain \eqref{co:general_unitary}, which often ends up far from optimal.
...
\begin{section}{Channel Singular Values Redistribution}
	...
	Next, we numerically characterize the attainable channel singular value region on top of Algorithm \ref{ag:rcg}.
	...
	\begin{subsection}{Toy Example}\label{sc:toy_example}
		...
		The \gls{d}-\gls{ris} is modeled by $\mathbf{\Theta}_\mathrm{D} = \mathrm{diag}(e^{\jmath \theta_1}, e^{\jmath \theta_2})$ while the fully-connected \gls{bd}-\gls{ris} can be modeled by 4 independent angular parameters
		\begin{equation}
	...
	\begin{subsection}{Some Analytical Bounds}\label{sc:bounds}
		...
		if the backward and backward channels are at least rank $k$, then
		...
		An example is given in Corollary \ref{co:los} for \gls{los} channels (i.e., $k=1$)\footnote{A similar eigenvalue result for \gls{d}-\gls{ris} has been derived in \cite{Semmler2023}.}.
		...
		We highlight that Proposition \ref{pp:rank_deficient} is an asymptotic result and the inequalities apply to \gls{d}- and \gls{bd}-\gls{ris} with arbitrary number of elements, regardless of the presence of direct channel.
		...
		Proposition \ref{pp:rank_deficient} evaluates the ultimate channel shaping limits of passive \gls{ris} and provides a reference for choosing the number of scattering elements in a practical deployment scenario.
		...
		Proposition \ref{pp:fully_connected} states that if the direct channel is blocked and the \gls{bd}-\gls{ris} is fully-connected, the only singular value bounds on the equivalent channel are the singular value bounds on the product of unitary-transformed backward and forward channels.
		...
		\begin{equation*}
			\mathbf{H}_\mathrm{B} =
				...
				\end{bmatrix}.
		\end{equation*}
		It is obvious that any \gls{d}-\gls{ris} can only attain $\mathrm{sv}(\mathbf{H}) = [8, 3]^\mathsf{T}$ while a \gls{bd}-\gls{ris} $\mathbf{\Theta} = \begin{bsmallmatrix} 0 & 1 \\ 1 & 0 \end{bsmallmatrix}$ results in $\mathrm{sv}(\mathbf{H}) = [12, 2]^\mathsf{T}$.
		...
		Proposition \ref{pp:fully_connected} is important since it transforms the channel shaping question to a linear algebra question: \emph{How the singular values of matrix product are bounded by the singular values of its individual factors?}
		...
		Corollary \eqref{co:sv_bound_horn}, also recognized as a variation of Horn's inequality \cite{Bhatia2001}, is the most comprehensive channel singular value bound attainable by passive \gls{ris} when the direct channel is blocked.
		...
		Interested readers are referred to \cite[Chapter 16, 24]{Hogben2013} for more information.
		...
		\begin{corollary}[Lower bound on the product of last $k$ singular values\label{co:sv_product_smallest}]\footnote{Corollaries \ref{co:sv_smallest} and \ref{co:sv_product_smallest} are less informative when $\bar{N} \ne N$ (i.e., $N_\mathrm{T} = N_\mathrm{S} = N_\mathrm{R}$ being false) as the lower bounds would coincide at zero.}
			\begin{equation}
		...
		\begin{corollary}[Upper bound on the channel power gain\label{co:sum_power}]
			...
			The channel power gain is upper bounded by the sum of element-wise product of squared singular values of backward and forward channels
			\begin{equation}
		...
		To achieve equalities in Corollaries \ref{co:sv_largest} -- \ref{co:sum_power}, the \gls{ris} needs to completely align the singular vector spaces of $\mathbf{H}_\mathrm{B}$ and $\mathbf{H}_\mathrm{F}$.
		...
		\begin{equation}
			\mathbb{E}\bigl\{ \lVert \mathbf{H} \rVert _ \mathrm{F}^2 \bigr\} = \sum_{n=1}^N \int_0^\infty f_{\lambda_n^{\min(N_\mathrm{R},N_\mathrm{S})}}(x) d x \int_0^\infty f_{\lambda_n^{\min(N_\mathrm{S},N_\mathrm{T})}}(x) d x,
			\label{eq:power_gain_rayleigh}
		\end{equation}
		% \begin{equation}
		% 	\begin{split}
		% 		\mathbb{E}\bigl\{ \lVert \mathbf{H} \rVert _ \mathrm{F}^2 \bigr\}
		% 		& = \sum_{n=1}^N \int_0^\infty f_{\lambda_n^{\min(N_\mathrm{R},N_\mathrm{S})}}(x_n) d x_n \\
		...
		Proposition \ref{pp:fully_connected} and Corollaries \ref{co:sv_bound_horn} -- \ref{co:sum_power} provide a comprehensive answer to the shaping capability question in terms of singular value bounds and channel power gain limits.
...
\begin{section}{Simulation Results}\label{sc:simulation}
	...
	\begin{subsection}{Channel Singular Values Redistribution}
		...
		\begin{subsubsection}{Analytical Bounds and Numerical Results}
			...
			It comes purely from channel rearrangement in \eqref{eq:gain_rearrangement} and aligns with the asymptotic \gls{bd}-\gls{ris} scaling law derived for \gls{siso} in \cite[Equation 30]{Shen2020a}.
...
\begin{section}{Conclusion}
	...
	Extensive simulations show that the advantage of \gls{bd}-\gls{ris} stems from its superior space alignment and channel rearrangement capability, which scales with the number of elements, group size, and \gls{mimo} dimensions.
