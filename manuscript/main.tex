...
% \title{MIMO Channel Shaping and Rate Maximization via Beyond Diagonal RIS}
...
\begin{section}{Introduction}
	...
	% $\Re\{\cdot\}$ and $\mathrm{arg}(\cdot)$ take the real part and argument of a complex number, respectively.
	...
	% \footnote{Following footnote \ref{fn:symmetric}, we do not constrain the scattering matrix to be symmetric in the paper. One can enforce such by $\mathbf{\Theta} \gets (\mathbf{\Theta} + \mathbf{\Theta}^\mathsf{T})/2$ if necessary.}
	\begin{equation}
	...
	% We highlight that conventional \gls{d}-\gls{ris} is an extreme case of \gls{bd}-\gls{ris} with group size $L=1$.
	...
	\begin{equation}
		\label{eq:channel}
		% \mathbf{H} = \mathbf{H}_\mathrm{D} + \mathbf{H}_\mathrm{B} \mathbf{\Theta} \mathbf{H}_\mathrm{F} = \mathbf{H}_\mathrm{D} + \sum_g \underbrace{\mathbf{H}_{\mathrm{B},g} \mathbf{\Theta}_g \mathbf{H}_{\mathrm{F},g}}_{\triangleq \mathbf{H}_g} = \mathbf{H}_\mathrm{D} + \sum_g \mathbf{H}_g,
	...
	Let $\mathbf{H}_{\mathrm{B},g} = \mathbf{U}_{\mathrm{B},g} \mathbf{\Sigma}_{\mathrm{B},g} \mathbf{V}_{\mathrm{B},g}^\mathsf{H}$ and $\mathbf{H}_{\mathrm{F},g} = \mathbf{U}_{\mathrm{F},g} \mathbf{\Sigma}_{\mathrm{F},g} \mathbf{V}_{\mathrm{F},g}^\mathsf{H}$ be the \gls{svd} of the backward and forward channels, respectively.
	...
	% \begin{remark}
	We observe from \eqref{eq:channel_svd} that the \gls{bd}-\gls{ris} plays a crucial role in combining
	...
	% We conclude from \eqref{eq:channel_svd} that the scattering matrix of \gls{bd}-\gls{ris} provide two channel shaping potentials:
	% By analyzing \eqref{eq:channel_svd}, we conclude that the off-diagonal entries of the \gls{bd}-\gls{ris} scattering matrix provide two key potentials for \gls{mimo} channel shaping:
	...
	\begin{itemize}
		\item \emph{Branch matching:} It refers to pairing and combining the branches (i.e., entries) of $\mathbf{H}_{\mathrm{B},g}$ and $\mathbf{H}_{\mathrm{F},g}$ through unitary transformation $\mathbf{\Theta}_g$.
			% That is,
			% % entries of $\mathbf{H}_{\mathrm{B},g}$ and $\mathbf{H}_{\mathrm{F},g}$ by their strength.
			% % rearranging and recombining the backward and forward channel branches (i.e., entries of $\mathbf{H}_\mathrm{B}$ and $\mathbf{H}_\mathrm{F}$)
			% % associated with each group
			% % by their strength.
			% % This unique feature of \gls{bd}-\gls{ris} exploits \textcolor{red}{spatial diversity} effectively thanks to in-group connections.
			% % This unique feature of \gls{bd}-\gls{ris} exploits the \textcolor{red}{spatial diversity} by rearranging and combining the backward and forward channel branches within each group.
			...
			% % Thanks to in-group connections,
			% while \gls{bd}-\gls{ris} can exploit in-group connections and generalize it to
			% % this indirect channel gain to
			% $g_\mathrm{BD}=\sum_{g=1}^{G} \sum_{l=1}^{L} \lvert h_{\mathrm{B},g,\pi_{\mathrm{B},g}(l)} \rvert \lvert h_{\mathrm{F},g,\pi_{\mathrm{F},g}(l)} \rvert$,
			% % $\sum_{g=1}^{G} \sum_{l=1}^{L} \lvert h_{\mathrm{B},\pi_{\mathrm{B},g}(l)} \rvert \lvert h_{\mathrm{F},\pi_{\mathrm{F},g}(l)} \rvert$,
			% % $\sum_{g=1}^{G} \sum_{l=1}^{L} \lvert h_{\mathrm{B},g,l} \rvert \lvert h_{\mathrm{F},g,\pi_g(l)} \rvert$,
			% where $\pi_{\mathrm{B},g}$ and $\pi_{\mathrm{F},g}$ are arbitrary permutations of $[L]$.
			...
			% % are the descending permutations of $\lvert h_{\mathrm{B},g,l} \rvert$ and $\lvert h_{\mathrm{F},g,l} \rvert$, respectively.
			...
			% % \footnote{Similarly, the minimum is attained when we pair the $l$-th strongest backward branch and the $L{-}l{+}1$-th strongest forward branch within each group.}
			% % We thus conclude that \gls{bd}-\gls{ris} exploits \textcolor{red}{spatial diversity} effectively thanks to in-group connections.
			...
			% % , providing up to \qty{62.5}{\percent} asymptotic power gain over \gls{d}-\gls{ris} in \gls{siso} Rayleigh fading \cite{Shen2020a}.
		...
		\item \emph{Mode alignment:} It refers to aligning and ordering the modes (i.e., singular vectors) of $\{\mathbf{H}_g\}_{g \in [G]}$ with those of $\mathbf{H}_\mathrm{D}$ through unitary transformations $\{\mathbf{\Theta}_g\}_{g \in [G]}$.
			% The \gls{bd}-\gls{ris} needs to strike a balance between the space alignment of backward-forward (intra-group, multiplicative) channels and direct-indirect (inter-group, additive) channels. In \gls{siso}, singular vectors reduce to scalars and space alignment reduce to phase matching, such that the scattering matrix of group $g$ maximizing the channel gain is
			% \begin{equation}
			...
			% We notice that \gls{d}-\gls{ris} (i.e., $L=1$) suffices
			% % \gls{d}-\gls{ris} thus suffices
			% for perfect phase matching in \gls{siso}.
			% When it comes to \gls{mimo}, each \gls{d}-\gls{ris} element can only apply a scalar phase shift to the corresponding indirect channel matrix of size $N_\mathrm{R} \times N_\mathrm{T}$ defined in \eqref{eq:channel}.
			% We thus conclude that the disadvantage of \gls{d}-\gls{ris} in space alignment deteriorates with \gls{mimo} dimensions.
			% % When it comes to \gls{mimo}, element $g$ of \gls{d}-\gls{ris} can only apply a common phase shift to the ``pinhole''\footnote{The pinhole effect refers to channel rank reduction due to diffraction in the propagation environment \cite{Almers2006}. For element $g$ of \gls{d}-\gls{ris}, the indirect channel $\mathbf{H}_g$ is rank-1 and the pinhole is $\Theta_g$.} indirect channel passing through itself by $\mathbf{H}_g = \mathbf{h}_{\mathrm{B},g} \Theta_g \mathbf{h}_{\mathrm{F},g}^\mathsf{H} \in \mathbb{C}^{N_\mathrm{R} \times N_\mathrm{T}}$.
			% % Thus, we conclude that
			% % We can thus see that
			% % {the disadvantage of \gls{d}-\gls{ris} in space alignment deteriorates with \gls{mimo} dimensions.}
	...
	% We use the following examples to show that a stronger branch matching capability enables better mode alignment and results in more flexible channel shaping.
	% The following example clarifies the difference in between and show their impacts on channel shaping.
	...
	\begin{example}[\gls{siso} channel gain maximization]
		...
		% the scattering matrix maximizing the channel gain is
		\begin{equation}
		...
		% Here any $L \in [N_\mathrm{S}]$ (including \gls{d}-\gls{ris}) suffices for perfect mode alignment between $h_\mathrm{D}$ and indirect channels $\{h_g\}_{g \in [G]}$ using \eqref{eq:scattering_siso}.
		% However, the branch matching capability is a function of $L$ and the channel gain can be manipulated
		\begin{equation}
		...
		% It is evident that the branch matching capability increases with $L$.
		% the backward and forward channels as $\mathbf{h}_\mathrm{B} \in \mathbb{C}^{N_\mathrm{S} \times 1}$ and $\mathbf{h}_\mathrm{F}^\mathsf{H} \in \mathbb{C}^{1 \times N_\mathrm{S}}$, whose $n$-th entries are $h_{\mathrm{B},n}$ and $h_{\mathrm{F},n}$, respectively.
		% The channels associated with \gls{bd}-\gls{ris} group $g$ are also denoted as $\mathbf{h}_{\mathrm{B},g} \in \mathbb{C}^{L \times 1}$ and $\mathbf{h}_{\mathrm{F},g}^\mathsf{H} \in \mathbb{C}^{1 \times L}$, whose $l$-th entries are $h_{\mathrm{B},g,l}$ and $h_{\mathrm{F},g,l}$, respectively.
	\end{example}
	...
	% of size $N_\mathrm{R} \times N_\mathrm{T}$, which renders the alignment of multiple modes difficult.
	% following examples aim to help the readers understanding both potentials.
	...
	% Interestingly, the gain surges to \qty{270}{\percent} in $N_\mathrm{T}=N_\mathrm{R}=4$ \gls{mimo} as shown in Fig. \subref*{fg:power_bond_txrx4_nd}.
	...
	% That is, \gls{d}-\gls{ris} is sufficient for space alignment in \gls{siso} while the \qty{62.5}{\percent} gain from \gls{bd}-\gls{ris} comes purely from channel rearrangement (i.e., pairing the forward and backward channels from strongest to weakest).
	...
	% \begin{remark}
	% 	From \eqref{eq:ris_decompose} and \eqref{eq:channel_nd} in the proof of Proposition \ref{pp:nd}, we notice that \eqref{iq:horn}--\eqref{iq:sv_bound_fc_power} are simultaneously tight when
	% 	% Interestingly, \eqref{iq:horn}--\eqref{iq:sv_bound_fc_power} are simultaneously tight when
	% 	\begin{equation}
	...
	% 	\begin{itemize}
	% 		\item space alignment: $\mathbf{V}_\mathrm{B}$ and $\mathbf{U}_\mathrm{F}^\mathsf{H}$ in \eqref{eq:ris_decompose} fully align the spaces of $\mathbf{H}_\mathrm{B}$ and $\mathbf{H}_\mathrm{F}$ by rotation;
	% 		\item channel rearrangement: $\mathbf{X} = \mathbf{I}$ in \eqref{eq:channel_nd} pairs the channels of $\mathbf{H}_\mathrm{B}$ and $\mathbf{H}_\mathrm{F}$ from strongest to weakest, which attains the maximum in rearrangement inequality.
	% 	\end{itemize}
	% \end{remark}
	...
	% \begin{remark}
	% 	\gls{bd}-\gls{ris} reduces to \gls{d}-\gls{ris} and fully-connected \gls{bd}-\gls{ris} with group size $L$ 1 and $N_\mathrm{S}$, respectively.
	% \end{remark}
	% \begin{remark}
	% 	Individual forward and backward \gls{csi} are required for \gls{bd}-\gls{ris} designs.
	% 	This is different from \gls{d}-\gls{ris} where estimating their product is usually sufficient.
	% 	% Later we will show the potential benefits from the \gls{csi} overhead.
	% \end{remark}
	\end{section}

	\begin{section}{Analytical Shaping Bounds}
		\label{sc:bounds}
	...
	% the channel singular values for both architectures.
	% Next, we numerically characterize the attainable channel singular value region using Algorithm \ref{ag:rcg}.
	% Finally, we derive
	% Some analytical bounds on the channel singular values are derived for \gls{bd}-\gls{ris} and \gls{d}-\gls{ris} architectures.
	% singular value bounds in specific channel conditions.

	\begin{example}[$2 \times 2 \times 2$ shaping]
		\label{eg:shaping_potential}
		% We showcase the channel shaping capabilities of different \gls{ris} models by a toy example.
		...
		% is modeled by $\mathbf{\Theta}_\mathrm{D} = \diag(e^{\jmath \theta_1}, e^{\jmath \theta_2})$ while the fully-connected \gls{bd}-\gls{ris} can be modeled by 4 independent angular parameters
		\begin{equation*}
			...
			\end{bmatrix},
		\end{equation*}
		% where $\phi$ has no impact on $\sv(\mathbf{H})$ since $\sv(e^{\jmath \phi} \mathbf{A}) = \sv(\mathbf{A})$.
		...
		\begin{equation*}
			...
			\end{split}
		\end{equation*}
		% The results are obtained by an exhaustive grid search over $(\theta_1, \theta_2)$ for \gls{d}-\gls{ris} and $(\alpha, \psi)$ for symmetric \gls{bd}-\gls{ris}.
	...
	% Example~\ref{eg:shaping_potential} shows that physically grouping some scattering elements produces a ``cooperation effect'' that significantly enhances the dynamic range of channel singular values.
	Example~\ref{eg:shaping_potential} highlights that the physical grouping of \gls{ris} elements, even if using symmetric components, can produce a much wider dynamic range of channel singular values.
	This motivates analytical and numerical studies on singular value shaping using \gls{bd}-\gls{ris}.
	A selection of analytical bounds are presented in Propositions~\ref{pp:dof} -- \ref{pp:nd} and the resulting corollaries.
	% The remainder of this section is organized as follows.

	% We also provide some analytical bounds related to channel singular values, which help us to explore the limits of channel shaping using passive \gls{ris}.
	...
	\begin{proposition}[Degrees of freedom]
		\label{pp:dof}
		In point-to-point \gls{mimo}, \gls{bd}-\gls{ris} may achieve a larger or smaller number of \gls{dof} than \gls{d}-\gls{ris}.
	\end{proposition}
	\begin{proof}
	...
	% you cannot hope for degrees of freedom gain (i.e. more channels) thus we focus ont he question of reshaping of the available channels through manipulation of the singular values.
	...
	% with a \gls{d}-\gls{ris} or \gls{bd}-\gls{ris} of sufficiently large number of scattering elements,
	...
	We emphasize that
	% the inequalities in \eqref{iq:sv_rd} and \eqref{iq:sv_los}
	Proposition \ref{pp:rd} and Corollary \ref{co:los} apply to both \gls{d}- and \gls{bd}-\gls{ris} configurations regardless of the status of the direct channel.
	Out of $2N$ bounds in \eqref{iq:sv_rd} or \eqref{iq:sv_los}, $N$ of them
	% The equalities in \eqref{iq:sv_rd} and \eqref{iq:sv_los}
	...
	% Proposition \ref{pp:rd} evaluates the ultimate channel shaping limits of passive \gls{ris} and provides a benchmark for $N_\mathrm{S}$ selection in low-multipath deployment scenarios.
	Next, we shift the focus to another popular \gls{ris} deployment scenario where the direct channel is blocked.
	% finite-size fully-connected \gls{bd}-\gls{ris}.
	...
	% define\footnote{This is equivalent to padding zero blocks at the end of $\mathbf{H}, \mathbf{H}_\mathrm{B}, \mathbf{H}_\mathrm{F}$ to make square matrices.} $\bar{N} = \max(N_\mathrm{T},N_\mathrm{S},N_\mathrm{R})$ and $\sigma_n(\mathbf{H})=\sigma_n(\mathbf{H}_\mathrm{F})=\sigma_n(\mathbf{H}_\mathrm{B})=0$ for $n \in [\bar{N}] \setminus [N]$.
	% where we assume $N \le N_\mathrm{S}$ for the ease of notation and define\footnote{This is equivalent to padding zero blocks at the end of $\mathbf{H}, \mathbf{H}_\mathrm{B}, \mathbf{H}_\mathrm{F}$ to make square matrices.} $\sigma_n(\mathbf{H})=\sigma_n(\mathbf{H}_\mathrm{F})=\sigma_n(\mathbf{H}_\mathrm{B})=0$ for $n \in [N_\mathrm{S}] \setminus [N]$.
	% \footnote{This is equivalent to padding zero blocks at the end of $\mathbf{H}$, $\mathbf{H}_\mathrm{B}$, and $\mathbf{H}_\mathrm{F}$ to make square matrices, which generalizes relevant results to non-square case.} $N = \max(N_\mathrm{T},N_\mathrm{S},N_\mathrm{R})$ and $\sigma_n(\mathbf{H})=\sigma_n(\mathbf{H}_\mathrm{F})=\sigma_n(\mathbf{H}_\mathrm{B})=0$ for $n \in [N] \setminus [N]$}.
	% over the notation\footnote{This is equivalent to padding zero blocks at the end of $\mathbf{H}$, $\mathbf{H}_\mathrm{B}$, and $\mathbf{H}_\mathrm{F}$ to make square matrices, which generalizes relevant results to non-square case.} $N = \max(N_\mathrm{T},N_\mathrm{S},N_\mathrm{R})$ and $\sigma_n(\mathbf{H})=\sigma_n(\mathbf{H}_\mathrm{F})=\sigma_n(\mathbf{H}_\mathrm{B})=0$ for $n \in [N] \setminus [N]$.
	% $N < n \le N$.
	...
	\eqref{iq:horn}, also recognized as a variation of Horn's inequality \cite{Bhatia2001}, is one of the most comprehensive result over Proposition \ref{pp:nd}.
	% It is worth mentioning that t
	% It also helps to induce lower bounds on the channel singular values.
	The number of admissible triples increases exponentially with $N_\mathrm{S}$ despite some resulting bounds can be redundant.
	Apart from the upper bounds, we will shortly see \eqref{iq:horn} can also induce lower bounds on channel singular values.
	% It also helps to induce lower (instead of upper) bounds on channel singular values.
	% ! For example, the number of inequalities described by \eqref{iq:horn} grows from 12 to 2062 when $N_\mathrm{S}$ increases from 3 to 7.
	% Besides, \eqref{iq:horn} helps to induce lower bounds
	...
	\begin{corollary}[Product of some largest or smallest singular values]
		...
		% \footnote{Corollaries \ref{co:sv_smallest} and \ref{co:sv_product_smallest} are less informative when $N \ne N$ (i.e., $N_\mathrm{T} = N_\mathrm{S} = N_\mathrm{R}$ being false) as the lower bounds would coincide at zero.}
		% For any $k \in [N]$,
		If the direct channel is negligible,
		% and $N \le N_\mathrm{S}$
		...
		% the product of the first (resp. last) $k$ singular values of $\mathbf{H}_\mathrm{B}$ and $\mathbf{H}_\mathrm{F}$ by
		% upper bounded by the product of first and last $k$ singular values of $\mathbf{H}_\mathrm{B}$ and $\mathbf{H}_\mathrm{F}$ by
		% $k$ singular values of $\mathbf{H}$ can be upper bounded by the product of first $k$ singular values of $\mathbf{H}_\mathrm{B}$ and $\mathbf{H}_\mathrm{F}$ by
		\begin{subequations}
			\begin{align}
	...
	% The assumption $N \le N_\mathrm{S}$
	...
	% Although Corollary \ref{co:nd_sv_indl} and Proposition \ref{pp:nd}
	% are derived under the assumption of negligible direct channel and fully-connected \gls{bd}-\gls{ris}, they are still useful for practical scenarios.
	...
	\begin{subequations}
		...
		\end{gather}
	\end{subequations}
	% where the arguments of $\max$ and $\min$ coincide at $\bigl\{\sigma_1(\mathbf{H}_\mathrm{B}) \sigma_{N_\mathrm{S}}(\mathbf{H}_\mathrm{F}), \sigma_2(\mathbf{H}_\mathrm{B}) \sigma_{N_\mathrm{S}-1}(\mathbf{H}_\mathrm{F}), \ldots, \sigma_{N_\mathrm{S}}(\mathbf{H}_\mathrm{B}) \sigma_1(\mathbf{H}_\mathrm{F})\bigr\}$.
	...
	\begin{example}[Bounds on $3 \times 3 \times 3$ shaping]
		...
		\begin{itemize}
			...
			\item \gls{bd}-\gls{ris}: According to \eqref{iq:sv_nd_indl}, a fully-connected \gls{bd}-\gls{ris} can manipulate the singular values up to
				% \begin{align*}
				% 	8 & \le \sigma_1(\mathbf{H}) \le 15, \\
				% 	4 & \le \sigma_2(\mathbf{H}) \le 10, \\
				% 	0 & \le \sigma_3(\mathbf{H}) \le 0,
				% \end{align*}
				\begin{equation*}
				...
				% for the upper and lower bounds of $\sigma_1(\mathbf{H})$, respectively.
				% \begin{equation}
				% 	\begin{bmatrix}
				% 		8 \\
				% 		4 \\
				...
				% 		\begin{equation}
				% 			\begin{alignedat}{2}
				% 				\max \bigl\{\sigma_1(\mathbf{H}_\mathrm{B}) \sigma_3(\mathbf{H}_\mathrm{F}), \sigma_2(\mathbf{H}_\mathrm{B}) \sigma_2(\mathbf{H}_\mathrm{F}), \sigma_3(\mathbf{H}_\mathrm{B}) \sigma_1(\mathbf{H}_\mathrm{F})\bigr\} &\le \sigma_1(\mathbf{H}) &&\le \sigma_1(\mathbf{H}_\mathrm{B}) \sigma_1(\mathbf{H}_\mathrm{F}), \\
				% 				\max \bigl\{\sigma_2(\mathbf{H}_\mathrm{B}) \sigma_3(\mathbf{H}_\mathrm{F}), \sigma_3(\mathbf{H}_\mathrm{B}) \sigma_2(\mathbf{H}_\mathrm{F})\bigr\} &\le \sigma_2(\mathbf{H}) &&\le \min \bigl\{\sigma_1(\mathbf{H}_\mathrm{B}) \sigma_2(\mathbf{H}_\mathrm{F}), \sigma_2(\mathbf{H}_\mathrm{B}) \sigma_1(\mathbf{H}_\mathrm{F})\bigr\}, \\
				% 				\sigma_3(\mathbf{H}_\mathrm{B}) \sigma_3(\mathbf{H}_\mathrm{F}) &\le \sigma_3(\mathbf{H}) &&\le \min \bigl\{\sigma_1(\mathbf{H}_\mathrm{B}) \sigma_3(\mathbf{H}_\mathrm{F}), \sigma_2(\mathbf{H}_\mathrm{B}) \sigma_2(\mathbf{H}_\mathrm{F}), \sigma_3(\mathbf{H}_\mathrm{B}) \sigma_1(\mathbf{H}_\mathrm{F})\bigr\}.
				% 			\end{alignedat}
				% 		\end{equation}
				% 		\begin{equation}
				% 			\begin{alignedat}{1}
				% 				\sigma_1(\mathbf{H}) \sigma_2(\mathbf{H}) & \le \sigma_1(\mathbf{H}_\mathrm{B}) \sigma_1(\mathbf{H}_\mathrm{F}) \sigma_2(\mathbf{H}_\mathrm{B}) \sigma_2(\mathbf{H}_\mathrm{F}), \\
				% 				\sigma_2(\mathbf{H}) \sigma_3(\mathbf{H}) & \le \sigma_2(\mathbf{H}_\mathrm{B}) \sigma_2(\mathbf{H}_\mathrm{F}) \sigma_3(\mathbf{H}_\mathrm{B}) \sigma_3(\mathbf{H}_\mathrm{F}),
				% 			\end{alignedat}
				% 		\end{equation}
				% 		\begin{equation}
				% 			\begin{alignedat}{1}
				% 				\sigma_1(\mathbf{H}) \sigma_3(\mathbf{H}) & \le \min \bigl\{\sigma_1(\mathbf{H}) \sigma_3(\mathbf{H}) \sigma_1(\mathbf{H}) \sigma_2(\mathbf{H}), \sigma_1(\mathbf{H}) \sigma_2(\mathbf{H}) \sigma_1(\mathbf{H}) \sigma_3(\mathbf{H})\bigr\}, \\
				% 				\sigma_2(\mathbf{H}) \sigma_3(\mathbf{H}) & \le \min \bigl\{\sigma_2(\mathbf{H}) \sigma_3(\mathbf{H}) \sigma_1(\mathbf{H}) \sigma_2(\mathbf{H}), \sigma_1(\mathbf{H}) \sigma_2(\mathbf{H}) \sigma_2(\mathbf{H}) \sigma_3(\mathbf{H}), \sigma_1(\mathbf{H}) \sigma_3(\mathbf{H}) \sigma_1(\mathbf{H}) \sigma_3(\mathbf{H})\bigr\}
				% 			\end{alignedat}
				% 		\end{equation}
				% 	\end{subequations}
				% \end{figure*}
				...
				% \end{align*}
		\end{itemize}
		% $\sv(\mathbf{H}_\mathrm{B}) = [3, 2]^\mathsf{T}$ and $\sv(\mathbf{H}_\mathrm{B}) = [4, 1]^\mathsf{T}$.
		% \begin{equation*}
		% 	\mathbf{H}_\mathrm{B} =
		% 		\begin{bmatrix}
		% 			4 & 0 & 0 \\
		...
		% 		\end{bmatrix}.
		% \end{equation*}
		% Evidently, any \gls{d}-\gls{ris} can only achieve $\sv(\mathbf{H}) = [12, 5, 0]^\mathsf{T}$ due to limited branch matching and mode ordering capability.
		% On the other hand, Corollary \ref{co:nd_sv_prod_subset} suggests that a fully-connected \gls{bd}-\gls{ris} can manipulate the singular values up to
		...
		% and the singular value region is a subset of the rectangle due to the trade-off between the alignment of two modes.
		% but there exists a trade-off in between due to mode alignment.
		% It is obvious that any \gls{d}-\gls{ris} can only attain $\sv(\mathbf{H}) = [8, 3]^\mathsf{T}$ while a \gls{bd}-\gls{ris} $\mathbf{\Theta} = \begin{bsmallmatrix} 0 & 1 \\ 1 & 0 \end{bsmallmatrix}$ exploits branch matching to achieve $\sv(\mathbf{H}) = [12, 2]^\mathsf{T}$.

		% results in $\sv(\mathbf{H}) = [12, 2]^\mathsf{T}$.
		% The singular value
	...
	% performing right unitary transformation on $\mathbf{H}_\mathrm{B}$ and left unitary transformation on $\mathbf{H}_\mathrm{F}$, which properly aligns and orders their modes.
	% properly aligning and ordering the modes of $\mathbf{H}_\mathrm{B}$ and $\mathbf{H}_\mathrm{F}$.
	% enables right unitary transformation on $\mathbf{H}_\mathrm{B}$ and left unitary transformation on $\mathbf{H}_\mathrm{F}$, widening the singular values
	% illustrates the shaping limits of a $3 \times 3 \times 3$ setup with $\mathbf{H}_\mathrm{D} = \mathbf{0}$, $\mathbf{H}_\mathrm{B} = \diag(3, 2, 1)$, and $\mathbf{H}_\mathrm{F} = \diag(4, 0, 5)$.
	% That is, fully-connected \gls{bd}-\gls{ris} applies right unitary transformation on $\mathbf{H}_\mathrm{B}$ and left unitary transformation on $\mathbf{H}_\mathrm{F}$, widening the singular values to $8 \le \sigma_1(\mathbf{H}) \le 12$ and $2 \le \sigma_2(\mathbf{H}) \le 3$.
	...
	We note that \eqref{eq:power_nd_rayleigh} generalizes the \gls{siso} power gain attainable by \gls{bd}-\gls{ris} \cite[(58)]{Shen2020a} to \gls{mimo} under double Rayleigh fading.

	\begin{corollary}[Channel capacity at very low and high \gls{snr}]
		\label{co:nd_capacity}
		...
		Their upper bounds can be attained by, for example, \eqref{eq:ris_nd_power_max}.
	\end{corollary}
	...
	Similar to \eqref{eq:power_nd_rayleigh}, the ergodic counterparts of \eqref{iq:capacity_nd_snr_low} and \eqref{iq:capacity_nd_snr_high} when both $\mathbf{H}_\mathrm{B}$ and $\mathbf{H}_\mathrm{F}$ follow Rayleigh fading can be numerically evaluated as
	\begin{subequations}
		\begin{equation}
	...
	% since the mode alignment and ordering are not guaranteed to be simultaneously achievable.
	% ose results to the case of direct channel is non-trivial since the singular vector spaces of direct-indirect channels and backward-forward channels cannot be aligned at the same time.
	...
	\begin{section}{Numerical Shaping Solutions}
		\label{sc:framework}
		% Consider a special class of channel shaping problem
		% \begin{maxi!}
		% 	{\scriptstyle{\mathbf{\Theta}}}{f(\mathbf{H})}{\label{op:shaping}}{\label{ob:shaping}}
		% 	\addConstraint{\mathbf{\Theta}_g^\mathsf{H} \mathbf{\Theta}_g=\mathbf{I},}{\quad \forall g,}{\label{cn:shaping_unitary}}
		% \end{maxi!}
		% where $f: \mathbb{C}^{N_\mathrm{R} \times N_\mathrm{T}} \to \mathbb{R}$ is a unitary invariant function satisfying $f(\mathbf{U}^\mathsf{H} \mathbf{H} \mathbf{V}) = f(\mathbf{H})$ for any $\mathbf{H} \in \mathbb{C}^{N_\mathrm{R} \times N_\mathrm{T}}$, $\mathbf{U} \in \mathbb{U}^{N_\mathrm{R} \times N_\mathrm{R}}$, $\mathbf{V} \in \mathbb{U}^{N_\mathrm{T} \times N_\mathrm{T}}$.
		% Examples of such $f$ include the $n$-th channel singular value, channel power gain, and channel capacity.
		% According to
		Consider a special class of channel shaping problem
		\begin{maxi!}
		...
		Problem \eqref{op:shaping} can be solved by a \gls{rcg} method over proposition \ref{pp:shaping}.
		% Although
		...
		\begin{proposition}
			...
			% $d_n \in \partial_{\sigma_n(\mathbf{H})} f\bigl(\sv(\mathbf{H})\bigr)$, $\forall n \in [N]$, and $\mathbf{U}$, $\mathbf{V}$ are any left and right singular matrices of $\mathbf{H}$.



			% One Euclidean sub-gradient of \eqref{ob:shaping} with respect to \gls{bd}-\gls{ris} block $g$ is
			% \begin{equation}
			% 	\frac{\partial f\bigl(\sv(\mathbf{H})\bigr)}{\partial \mathbf{\Theta}_g} = \sum_{n=1}^N \frac{\partial f\bigl(\sv(\mathbf{H})\bigr)}{\partial \sigma_n(\mathbf{H})} \mathbf{H}_{\mathrm{B},g}^\mathsf{H} \mathbf{u}_n \mathbf{v}_n^\mathsf{H} \mathbf{H}_{\mathrm{F},g}^\mathsf{H},
			% \end{equation}
			% where $\mathbf{u}_n$ and $\mathbf{v}_n$ are any (c.f. Footnote \ref{fn:svd_non_uniqueness}) left and right singular vectors of $\mathbf{H}$ corresponding to $\sigma_n(\mathbf{H})$.
		\end{proposition}
	...
	\addConstraint{\mathbf{\Theta}_g^\mathsf{H} \mathbf{\Theta}_g=\mathbf{I},}{\quad \forall g,}{\label{cn:general_unitary}}
	\end{maxi!}
	where the objective function $f(\mathbf{\Theta})$ is a function of the \gls{bd}-\gls{ris} scattering matrix, for example, channel singular value (to be discussed in Section \ref{sc:pareto}), channel power gain (to be discussed in Section \ref{sc:power}), and achievable rate (to be discussed in Section \ref{sc:rate}).
	...
	The former solves unconstrained problem \eqref{ob:general} by quasi-Newton methods then projects the solution back to domain \ref{cn:general_unitary}, which often ends up far from optimal.
	...
	% It has been widely adopted in \gls{d}- and \gls{bd}-\gls{ris} literature.
	% progressing along the conjugate direction and retracting back to the manifold.
	% refining the solution iteratively by progressing along the conjugate direction and projecting back to the manifold at each step.
	% which progresse along the conjugate direction and projecting back to the manifold at each step
	% updates the feasible point by progressing along the conjugate direction and projecting back to the manifold at each step.
	...
	% provide a brief overview on the general \gls{rcg} method used in \gls{bd}-\gls{ris} optimization, namely non-geodesic \gls{rcg}, and discuss its drawbacks.
	% explain the existing manifold RCG method used in BD-RIS optimization, namely non-geodesic RCG, and discuss its drawbacks,
	% first discuss the existing non-geodesic \gls{rcg} method and its drawbacks, then propose a novel group-wise geodesic \gls{rcg} method that operates directly on the Stiefel manifold for faster convergence.

	\begin{subsection}{Optimization}





		\label{sc:rcg_general}
		...
		% It is non-geodesic because the updates constitute a zigzag path aided by the Euclidean space instead of carried entirely on the manifold.
		% updates are not entirely on the manifold
		% we progress along a direct path in the tangent space and departs from the manifold in each step.
		% the updates are not entirely
		% , at each step, we progress along a direct path in the tangent space and departs from the manifold in each step.
		...
		\begin{enumerate}
		\item \emph{Compute the Euclidean gradient:} The gradient of $f$ with respect to $\mathbf{\Theta}_g$ in the Euclidean space is \cite[(A.3.6)]{Fischer2005}
			\begin{equation}
		...
		\item \emph{Retract for feasibility \cite{Absil2009,Li2023b}:} The resulting point needs to be projected to the closest point (in terms of Euclidean distance) on the Stiefel manifold by
			\begin{equation}
				\label{eq:iterate_retr}
				\mathbf{\Theta}_g^{(r+1)} = \bar{\mathbf{\Theta}}_g^{(r+1)} \bigl({\bar{\mathbf{\Theta}}_g^{(r+1)\mathsf{H}}} \bar{\mathbf{\Theta}}_g^{(r+1)}\bigr)^{-1/2}.
			\end{equation}
			One can also combine the addition \eqref{eq:iterate_add} and retraction \eqref{eq:iterate_retr} in one step
			\begin{equation}
				\label{eq:iterate_comb}
	...
	The method is called non-geodesic since the addition \eqref{eq:iterate_add} and retraction \eqref{eq:iterate_retr} constitute a zigzag path departing from and returning to the manifold.
	% aided by the Euclidean space instead of carried entirely on the manifold.
	% the points are updated in the linear embedding spaces by addition \eqref{eq:iterate_add} and retraction \eqref{eq:iterate_retr}, instead of on the Stiefel manifold itself.
	...
	% usually suffers from numerical instability
	% additive update \eqref{eq:iterate_add} in the Euclidean space and retraction \eqref{eq:iterate_retr}.
	% Those
\end{subsection}
...
\begin{subsection}{Geodesic RCG}
	...
	During the optimization of group $g$, the geodesic evaluated at the identity \eqref{eq:geodesic_iden} should be translated to $\mathbf{\Theta}_g^{(r)}$ for successive updates \cite{Abrudan2008}
	\begin{equation}
		\label{eq:geodesic_tran}
		\mathbf{G}_g^{(r)}(\mu) = \mathbf{G}_\mathbf{I}(\mu) \mathbf{\Theta}_g^{(r)} = \exp(\mu \mathbf{D}_g^{(r)}) \mathbf{\Theta}_g^{(r)},
	\end{equation}
	while the Riemannian gradient evaluated at $\mathbf{\Theta}_g^{(r)}$ \eqref{eq:gradient_riem} should be translated back to the identity for exploiting the Lie algebra \cite{Abrudan2008}
	\begin{equation}
		...
		% $\mathcal{O}(2 L^3)$
	\end{equation}
	After gradient translation, the deviation parameter and conjugate direction can be determined similarly to \eqref{eq:conjugate_parm} and \eqref{eq:conjugate_dirn}
	\begin{equation}
	...
	\begin{algorithm}[!t]
		...
		\begin{algorithmic}[1]
			...
			\Repeat
			\For {$g \gets 1$ to $G$}
			\State $\nabla_{\mathrm{E},g}^{(r)} \gets$ \eqref{eq:gradient_eucl}
			\State $\tilde{\nabla}_{\mathrm{R},g}^{(r)} \gets$ \eqref{eq:gradient_riem_tran}
			\State $\tilde{\gamma}_g^{(r)} \gets$ \eqref{eq:conjugate_parm_geod}
			\State $\mathbf{D}_g^{(r)} \gets$ \eqref{eq:conjugate_dirn_geod}
			...
			\EndIf
			\State $\mu \gets 1$
			\State $\mathbf{G}_g^{(r)}(\mu) \gets$ \eqref{eq:geodesic_tran}
			\While {$\scriptstyle f\bigl(\mathbf{G}_g^{(r)}(2\mu)\bigr) - f(\mathbf{\Theta}_g^{(r)}) \ge \mu \cdot \tr(\mathbf{D}_g^{(r)} {\mathbf{D}_g^{(r)\mathsf{H}}}) / 2$} \label{ln:armijo_start}
			\State $\mu \gets 2 \mu$
			\EndWhile
			\While {$\scriptstyle f\bigl(\mathbf{G}_g^{(r)}(\mu)\bigr) - f(\mathbf{\Theta}_g^{(r)}) < \mu / 2 \cdot \tr(\mathbf{D}_g^{(r)} {\mathbf{D}_g^{(r)\mathsf{H}}}) / 2$}
			\State $\mu \gets \mu / 2$
			\EndWhile \label{ln:armijo_end}
			\State $\mathbf{\Theta}_g^{(r+1)} \gets$ \eqref{eq:iterate_mul}
			\EndFor
			\State $r \gets r+1$
	...
	Compared to the general non-geodesic approach, it leverages the Lie group properties to replace the add-then-retract update \eqref{eq:iterate_comb} with a multiplicative rotational update \eqref{eq:iterate_mul} along geodesics of the Stiefel manifold.
	...
	Since the set of block-unitary matrices are algebraically closed under multiplication, one can also avoid group-wise updates by directly operating on $\mathbf{\Theta}$ and pinching (i.e., keeping the main block diagonal and nulling others) the Euclidean gradient \eqref{eq:gradient_eucl}, which can further accelerate the algorithm when the number of group is large.
\end{subsection}
...
\begin{subsection}{Pareto Frontier Characterization}
	\label{sc:pareto}
We now characterize the Pareto frontier of singular values of a general $N_\mathrm{T} \times N_\mathrm{S} \times N_\mathrm{R}$ channel \eqref{eq:channel} by maximizing their weighted sum
\begin{maxi!}
...
\end{maxi!}
where $\rho_n \ge 0$ is the weight of the $n$-th singular value.
...
It can be solved optimally by Algorithm~\ref{ag:rcg} with the Euclidean gradient given in Lemma \ref{lm:pareto}.
% It can be solved optimally by Algorithm~\ref{ag:rcg} where line \ref{ln:gradient_euclidean} uses the following Euclidean gradient expression \eqref{eq:gradient_eucl_pareto}.

% \begin{lemma}
% 	\label{lm:pareto}
% 	The Euclidean gradient of \eqref{ob:pareto} with respect to \gls{bd}-\gls{ris} group $g$ is
% 	\begin{equation}
% 		\label{eq:gradient_eucl_pareto}
...
% 	\end{equation}
% 	where $\mathbf{U}$ and $\mathbf{V}$ are any left and right compact singular matrices of $\mathbf{H}$, respectively.
% \end{lemma}

\begin{lemma}
	\label{lm:pareto}
	One Euclidean sub-gradient of \eqref{ob:pareto} with respect to \gls{bd}-\gls{ris} group $g$ is
	\begin{equation}
		\label{eq:gradient_eucl_pareto}
		% \frac{\partial \sum_n \rho_n \sigma_n(\mathbf{H})}{\partial \mathbf{\Theta}_g^*} =
		\mathbf{H}_{\mathrm{B},g}^\mathsf{H} \mathbf{U} \diag(\rho_1,\ldots,\rho_N) \mathbf{V}^\mathsf{H} \mathbf{H}_{\mathrm{F},g}^\mathsf{H} \in \frac{\partial \sum_{n=1}^N \rho_n \sigma_n(\mathbf{H})}{\partial \mathbf{\Theta}_g^*}
	\end{equation}
	where $\mathbf{U}$ and $\mathbf{V}$ are any left and right singular matrices of $\mathbf{H}$, respectively.
\end{lemma}
\begin{proof}
	Please refer to Appendix~\ref{ap:pareto}.
\end{proof}
...
To update each \gls{bd}-\gls{ris} group, \gls{svd} of $\mathbf{H}$ requires $\mathcal{O}(N N_\mathrm{T} N_\mathrm{R})$, Euclidean gradient \eqref{eq:gradient_eucl_pareto} requires $\mathcal{O}\bigl(L N (N_\mathrm{T}+N_\mathrm{R}+L+1) \bigr)$, Riemannian gradient translation \eqref{eq:gradient_riem_tran} requires $\mathcal{O}(L^3)$, deviation parameter \eqref{eq:conjugate_parm_geod} and conjugate direction \eqref{eq:conjugate_dirn_geod} together require $\mathcal{O}(L^2)$, and matrix exponential \eqref{eq:iterate_mul} requires $\mathcal{O}(L^3)$ operations \cite{Moler2003}.
...
\end{equation}
where $\mathbf{U}$ and $\mathbf{V}$ are respectively any left and right singular matrices of $\mathbf{H}_\mathrm{B}^\dagger \mathbf{H}_\mathrm{D} \mathbf{H}_\mathrm{F}^\mathsf{H}$ or $\mathbf{H}_\mathrm{B}^\mathsf{H} \mathbf{H}_\mathrm{D} \mathbf{H}_\mathrm{F}^\dagger$ \cite{Golub2013}.
\end{remark}
...
\begin{proposition}
	...
	\end{equation}
	converges to a stationary point of \eqref{op:power}, where $\mathbf{U}_g^{(r)}$ and $\mathbf{V}_g^{(r)}$ are any left and right singular matrices of
	\begin{equation}
...
To update each \gls{bd}-\gls{ris} group, matrix multiplication \eqref{eq:auxiliary_power} requires $\mathcal{O}\bigl(N_\mathrm{T} N_\mathrm{R} + (G+1)(NL^2+N_\mathrm{T} N_\mathrm{R} L)\bigr)$ operations and its \gls{svd} requires $\mathcal{O}(L^3)$ operations.
...
\begin{subsection}{Proof of Proposition \ref{pp:dof}}
	...
	% Denote the rank of the
	% Let $r_\mathrm{B}$ and $r_\mathrm{F}$ be the ranks of the backward and forward channels, respectively.
	...
	% \begin{equation}
	% 	\max(r_\mathrm{B} - r_\mathrm{F}, 0) \le r_\mathrm{L}(\mathbf{\Theta}) \le \min(N_\mathrm{S} - r_\mathrm{F}, r_\mathrm{B}),
	% \end{equation}
	% which implies
	\begin{equation}
	...
	\end{equation}
	To attain the upper bound in \eqref{iq:rank_indirect}, the \gls{ris} needs to minimize $r_\mathrm{L}(\mathbf{\Theta})$ by aligning the ranges of $\mathbf{\Theta} \mathbf{U}_{\mathrm{F},2}$ and $\mathbf{V}_{\mathrm{B},2}$ as possible.
	This is achieved by
	\begin{equation}
...
\begin{subsection}{Proof of Corollary \ref{co:nd_sv_indl}}
	...
	% In \eqref{iq:sv_nd_indl}, the set of upper bounds
	% \begin{equation}
	% 	\bigl\{\sigma_n(\mathbf{H}) \le \sigma_i(\mathbf{H}_\mathrm{B}) \sigma_j(\mathbf{H}_\mathrm{F}) \mid [i,j,k] \in [N]^3, i+j=n+1\bigr\}
	% 	\label{iq:sv_nd_indl_set_max}
	% \end{equation}
	% is a special case of \eqref{iq:horn} with $(I, J, K) \in [N]^3$.
	% The minimum\footnote{One may think to take the \emph{maximum} of those upper bounds as the problem of interest is the attainable dynamic range of $n$-th singular value. However, this is infeasible since the singular values will be reordered therein.} of \eqref{iq:sv_nd_indl_set_max} is selected as the tightest upper bound in \eqref{iq:sv_nd_indl}.
	% On the other hand, the set of lower bounds
	% \begin{equation}
	% 	\bigl\{\sigma_n(\mathbf{H}) \ge \sigma_i(\mathbf{H}_\mathrm{B}) \sigma_j(\mathbf{H}_\mathrm{F}) \mid [i,j,k] \in [N]^3, i+j=n+N\bigr\}
	% 	\label{iq:sv_nd_indl_set_min}
	% \end{equation}
	% can be induced by \eqref{iq:sv_nd_indl_set_max}, \eqref{eq:sv_product_all}, and the non-negativity of singular values.
	% The maximum of \eqref{iq:sv_nd_indl_set_min} is selected as the tightest lower bound in \eqref{iq:sv_nd_indl}.

	% To attain the upper bound of $\sigma_n(\mathbf{H})$, the \gls{bd}-\gls{ris} needs to maximize the minimum of the first $n$ channel singular values.
	% This is achieved by proper matching of the first $n$ modes of $\mathbf{H}_\mathrm{B}$ to the first $n$ modes of $\mathbf{H}_\mathrm{F}$.
	% In particular, mode $i$ of $\mathbf{H}_\mathrm{B}$ should be completely aligned with mode $j$ of $\mathbf{H}_\mathrm{F}$ where $(i, j) = \arg \min_{i+j=n+1} \sigma_i(\mathbf{H}_\mathrm{B}) \sigma_j(\mathbf{H}_\mathrm{F})$ for $\mathbf{P}$

	% % Interested readers are referred to \cite[Equation~2.0.3]{Zhang2005} for more information.
	% % $i$-th mode of $\mathbf{H}_\mathrm{B}$ with the $j$-th mode of $\mathbf{H}_\mathrm{F}$ such that the $n$-th mode of $\mathbf{H}$ is maximized.

	% % Maximizing $\sigma_n(\mathbf{H}) = \sigma_n(\mathbf{H}) \le \min_{i+j=n+1} \sigma_i(\mathbf{H}_\mathrm{B}) \sigma_j(\mathbf{H}_\mathrm{F})$ is equivalent to maximizing the minimum of the first $n$ channel singular values.
	% % As such, the \gls{bd}-\gls{ris} needs to align t
	% % align the modes of $\mathbf{H}_\mathrm{B}$ and $\mathbf{H}_\mathrm{F}$ such that the first $n$ modes of $\mathbf{H}$ are maximized.
	% % To maximize the $n$-th channel singular value, the \gls{bd}-\gls{ris} needs to ensure
	% % that the first $n$ modes of $\mathbf{H}$ are

	% % should align the modes of $\mathbf{H}_\mathrm{B}$ and $\mathbf{H}_\mathrm{F}$ such that

	% % \cite[Equation~2.0.3]{Zhang2005}
\end{subsection}
...
\begin{subsection}{Proof of Corollary \ref{co:nd_capacity}}
	...
	We then show \eqref{eq:ris_nd_power_max} can achieve the upper bounds in \eqref{iq:capacity_nd_snr_low} and \eqref{iq:capacity_nd_snr_high} simultaneously.
	On the one hand, \eqref{eq:ris_nd_power_max} is a special case of \eqref{eq:ris_nd_sv_indl_max} with $\mathbf{P} = \mathbf{I}$, which satisfies \eqref{eq:idx_nd_sv_indl_max} and \eqref{eq:perm_nd_sv_indl_max} for $n=1$ and thus attain $\sigma_1(\mathbf{H}) = \sigma_1(\mathbf{H}_\mathrm{B}) \sigma_1(\mathbf{H}_\mathrm{F})$.
	On the other hand, since $\log(\cdot)$ is a monotonic function, we can prove similar to Appendix \ref{ap:nd_power} that $\sum_{n=1}^N \log \sigma_n^2(\mathbf{H}) \le \sum_{n=1}^N \log \sigma_n^2(\mathbf{H}_\mathrm{B}) \sigma_n^2(\mathbf{H}_\mathrm{F})$ and the bound is tight at \eqref{eq:ris_nd_power_max}.
...
\begin{subsection}{Proof of Proposition \ref{pp:shaping}}
	...
	\end{equation}
	% $$\mathbf{H} = \mathbf{H}_\mathrm{D} + \sum_g \mathbf{H}_{\mathrm{B},g} \mathbf{\Theta}_g \mathbf{H}_{\mathrm{F},g}$$
	where $\mathbf{D} \in \mathbb{C}^{N_\mathrm{R} \times N_\mathrm{T}}$ is a rectangular diagonal matrix with $[\mathbf{D}]_{n,n} \in \partial_{\sigma_n(\mathbf{H})} f\bigl(\sv(\mathbf{H})\bigr)$, $\forall n \in [N]$, and $\mathbf{U}$, $\mathbf{V}$ are any left and right singular matrices of $\mathbf{H}$.
	It implies
	\begin{align*}
...
\begin{subsection}{Proof of Lemma \ref{lm:pareto}}
	\label{ap:pareto}
	The sub-differential of $\sigma_n(\mathbf{H})$ with respect to $\mathbf{H}$ is given by
	\begin{equation*}
		\partial \sigma_n(\mathbf{H}) = \mathbf{u}_n \mathbf{v}_n^\mathsf{H},
	\end{equation*}




	Let $\mathbf{H} = \sum_n \mathbf{u}_n \sigma_n \mathbf{v}_n^\mathsf{H}$ be the \gls{svd} of the equivalent channel.
	...
	Let $f(\mathbf{\Theta}_g) = \sum_{n=1}^N \rho_n \sigma_n(\mathbf{H})$. Its sub-differential is defined as
	\begin{equation}
		\Bigl\{\mathbf{Y} \mid f(\tilde{\mathbf{\Theta}}_g) \ge f(\mathbf{\Theta}_g) + \tr\bigl(\mathbf{Y}^\mathsf{H} (\tilde{\mathbf{\Theta}}_g - \mathbf{\Theta}_g)\bigr), \forall \tilde{\mathbf{\Theta}}_g \in \mathbb{U}^{L \times L}\Bigr\}.
	\end{equation}

	whose sub-gradient with respect to $\mathbf{\Theta}_g^*$ is
	\begin{align*}
		\partial \sigma_n
	...
	\begin{equation}
		\label{eq:gradient_eucl_sv_indl}
	...
	\end{equation}
	A linear combination of \eqref{eq:gradient_eucl_sv_indl} yields \eqref{eq:gradient_eucl_pareto}.
\end{subsection}
...
\end{equation}
% For a given $\mathbf{\Theta}$, we approximate the quadratic objective \eqref{ob:power} by local Taylor expansion and recast problem \eqref{op:power} as

First, we approximate the quadratic objective \eqref{ob:power} by its local Taylor expansion
\begin{maxi!}
...
\addConstraint{\mathbf{\Theta}_g^\mathsf{H} \mathbf{\Theta}_g=\mathbf{I}, \quad \forall g.}{}{}
\end{maxi!}
Let $\mathbf{M}_g = \mathbf{U}_g \mathbf{\Sigma}_g \mathbf{V}_g^\mathsf{H}$ be the \gls{svd} of $\mathbf{M}_g$.
We have
\begin{equation}
...
\end{equation}
The upper bound is tight when $\mathbf{V}_g^\mathsf{H} \mathbf{\Theta}_g^\mathsf{H} \mathbf{U}_g = \mathbf{I}$, which implies the optimal solution of \eqref{op:power_ris_taylor} is $\tilde{\mathbf{\Theta}}_g = \mathbf{U}_g \mathbf{V}_g^\mathsf{H}$, $\forall g$.

Next, we prove that solving \eqref{op:power_ris_taylor} successively does not decrease \eqref{ob:power}.
Since $\tilde{\mathbf{\Theta}}$ optimal for problem \eqref{op:power_ris_taylor}, we have $\sum_g 2 \Re\bigl\{ \tr(\tilde{\mathbf{\Theta}}_g^\mathsf{H} \mathbf{M}_g) \bigr\} \ge \sum_g 2 \Re\bigl\{ \tr(\mathbf{\Theta}_g^\mathsf{H} \mathbf{M}_g) \bigr\}$ which is explicitly expressed by \eqref{iq:power_ris_taylor}.
% \begin{equation}
...
\end{multline}
% Expanding both side gives \eqref{iq:power_ris_taylor}.
On the other hand, expanding $\lVert \sum_g \mathbf{H}_{\mathrm{B},g} \tilde{\mathbf{\Theta}}_g \mathbf{H}_{\mathrm{F},g} - \sum_g \mathbf{H}_{\mathrm{B},g} \mathbf{\Theta}_g \mathbf{H}_{\mathrm{F},g} \rVert _\mathrm{F}^2 \ge 0$ gives \eqref{iq:auxiliary_nonnegative}.
\begin{multline}
...
\end{multline}
which suggests that updating $\tilde{\mathbf{\Theta}}$ does not decrease \eqref{ob:power}.
% \begin{figure*}
% \begin{equation}
% 	\label{iq:power_ris_taylor}
...
Finally, we prove that the converging point of \eqref{op:power_ris_taylor}, denoted by $\tilde{\mathbf{\Theta}}^?$, is a stationary point of \eqref{op:power}.
...
\end{gather}
On convergence, \eqref{eq:power_ris_taylor_optimal} becomes $\mathbf{H}_{\mathrm{B},g}^\mathsf{H} (\mathbf{H}_\mathrm{D} + \mathbf{H}_\mathrm{B} \mathbf{\Theta}^? \mathbf{H}_\mathrm{F}) \mathbf{H}_{\mathrm{F},g}^\mathsf{H} - \mathbf{\Theta}_g^? \mathbf{\Lambda}_g^\mathsf{H} = 0$ and reduces to \eqref{eq:power_ris_optimal}.
...
% 		Interestingly, the result suggests that \gls{bd}-\gls{ris} can achieve a higher \gls{dof} than \gls{d}-\gls{ris} in \gls{mimo}-\gls{ic}, which is not the case in \gls{mimo}-\gls{pc} (as discussed in \ref{sc:pc_rank_deficient}).
% 	\end{subsection}
...
% 		% 	\begin{split}
% 		% 		\lambda_k^{(r)}
% 		% 		& = \frac{\sum_{j \ne k} \tr(\mathbf{\Omega}_k^{(r)}\mathbf{F}_{kj}^{(r)} {\mathbf{F}_{kj}^{(r)\mathsf{H}}}) - \sum_{j \ne k} \tr(\mathbf{\Omega}_j^{(r)}\mathbf{F}_{jk}^{(r)} {\mathbf{F}_{jk}^{(r)\mathsf{H}}})}{P_k} \\
% 		% 		& \quad + \frac{\tr(\eta \mathbf{\Omega}_k^{(r)} \mathbf{G}_k^{(r)}{\mathbf{G}_k^{(r)\mathsf{H}}})}{P_k}
% 		% 	\end{split}
% 		% \end{equation}
% 		\begin{equation}
% 			\lambda_k^{(r)} = \frac{\tr\bigl(\eta \mathbf{\Omega}_k^{(r)} \mathbf{G}_k^{(r)}{\mathbf{G}_k^{(r)\mathsf{H}}} + \sum_j \mathbf{\Omega}_k^{(r)}\mathbf{T}_{kj}^{(r)} {\mathbf{T}_{kj}^{(r)\mathsf{H}}} - \mathbf{\Omega}_j^{(r)}\mathbf{T}_{jk}^{(r)} {\mathbf{T}_{jk}^{(r)\mathsf{H}}} \bigr)}{P_k},
% 		\end{equation}
