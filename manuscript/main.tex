...
\begin{section}{Introduction}
		...
		\begin{itemize}
			\item The $(i, j)$-th entry is $1$, where
				...
				\begin{subnumcases}{(i, j) =}
					\ \underset{\mathclap{i+j=n+1}}{\arg\min} \ \sigma_i(\mathbf{H}_\mathrm{B}) \sigma_j(\mathbf{H}_\mathrm{F}) & for $\mathbf{P}$, \label{eq:sv_nth_max_pair} \\
					\ \underset{\mathclap{i+j=n+N_\mathrm{S}}}{\arg\max} \ \sigma_i(\mathbf{H}_\mathrm{B}) \sigma_j(\mathbf{H}_\mathrm{F}) & for $\mathbf{Q}$, \label{eq:sv_nth_min_pair}
				\end{subnumcases}
				% \begin{equation*}
				% 	(i, j) =
				% 	\begin{cases}
	...
	\begin{section}{BD-RIS Design Frameworks}
