...
\begin{section}{Channel Shaping}
	...
	\begin{subsection}{Numerical Shaping Solution}
		...
		The latter generalizes the conjugate gradient methods to Riemannian manifolds and updates the solution by addition and retraction, constituting a zigzag path departing from and returning to the manifold.
		...
		It is applicable to a wide range of \gls{bd}-\gls{ris} design problems where the objective $f$ is smooth or convex non-smooth (not necessarily the symmetric gauge function \eqref{ob:shaping}) and the only constraint is group-wise unitarity \eqref{cn:shaping_unitary}.
		...
		\begin{algorithm}[!t]
			...
			\begin{algorithmic}[1]
				...
				\If {$\Re\bigl\{\tr({\mathbf{D}_g^{(r)\mathsf{H}}} \tilde{\nabla}_{\mathrm{R},g}^{(r)})\bigr\} < 0$} \Comment{Non-ascent}
				\State $\mathbf{D}_g^{(r)} \gets \tilde{\nabla}_{\mathrm{R},g}^{(r)}$
				\EndIf
				\State $\mu \gets 0.1$ \Comment{Doubling cheaper than halving}
				\State $\mathbf{G}_{\mathbf{\Theta}_g^{(r)}}(\mu) \gets$ \eqref{eq:geodesic_tran}
				\While {$\scriptstyle f(\mathbf{G}_{\mathbf{\Theta}_g^{(r)}}(2\mu)) - f(\mathbf{\Theta}_g^{(r)}) \ge \mu \cdot \tr(\mathbf{D}_g^{(r)} {\mathbf{D}_g^{(r)\mathsf{H}}}) / 2$} \label{ln:armijo_start}
				\State $\mu \gets 2 \mu$
				\EndWhile
				\While {$\scriptstyle f(\mathbf{G}_{\mathbf{\Theta}_g^{(r)}}(\mu)) - f(\mathbf{\Theta}_g^{(r)}) < \mu / 2 \cdot \tr(\mathbf{D}_g^{(r)} {\mathbf{D}_g^{(r)\mathsf{H}}}) / 2$}
				\State $\mu \gets \mu / 2$
		...
		Algorithm~\ref{ag:rcg} summarizes the proposed group-wise geodesic \gls{rcg} method for group-connected \gls{bd}-\gls{ris} design when the objective is smooth or convex non-smooth and the only constraint is group-wise unitarity.
...
\end{section}
	...
	\begin{section}{Rate Maximization}
		...
		\begin{subsection}{MIMO Point-to-Point Channel}
			...
			\end{equation}
			where $\mathbf{V}$ is the right singular matrix of the equivalent channel and $\mathbf{s}^\star$ is the optimal water-filling power allocation \cite{Clerckx2013}.
			...
			\begin{subsubsection}{Low-Complexity Solution}
				...
				\begin{paragraph}{Channel Power Gain}
					\label{sc:power}
