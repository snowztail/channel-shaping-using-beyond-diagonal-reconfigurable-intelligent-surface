...
\begin{abstract}
	...
	% in two common scenarios: 1) backward/forward channel is rank-deficient and 2) direct channel is blocked.
...
\begin{section}{Introduction}
	...
	\begin{subsection}{Related Works}
		At a specific resource block, channel shaping metrics can be categorized into two types.
	...
	\begin{subsection}{Contributions}
		...
		% For rank-deficient backward/forward channel,
		When the backward/forward channel is rank-deficient, we derive singular value bounds applying to \gls{d}- and \gls{bd}-\gls{ris} with asymptotically large number of elements.
		...
		% 1) rank-deficient backward/forward channel and 2) direct channel being blocked.
		...
		% % To explore the ultimate shaping limits, we derive individual and collective singular value bounds for rank-deficient backward/forward channel and blocked direct channel.
		...
		% Asymptotic channel singular value bounds applying to diagonal and \gls{bd}-\gls{ris} are derived for rank-deficient backward/forward channel.
...
\begin{section}{Simulation Results}
	...
	Unless otherwise specified, we assume $\kappa = 0$ (i.e., Rayleigh fading) for all channels and $\eta = \qty{-75}{dB}$ for all receivers.
	...
	For interference channel, we assume $K$ transmitters and receivers are independently and uniformly distributed in a disk of radius \qty{20}{m}, and all transmissions are of equal weight $\rho_k=1$, $\forall k$.
	...
	\begin{subsection}{Algorithm Evaluation}
		\begin{table*}[!t]
			...
			\end{tabular}
		\end{table*}
		Table \ref{tb:complexity} compares the geodesic \gls{rcg} method in Algorithm \ref{ag:rcg} and the non-geodesic \gls{rcg} method used in \cite{Li2023b,Li2023c,Zhou2023} on problem \eqref{op:pareto} where $N_\mathrm{T}=N_\mathrm{R}=4$ and $L=4$.
		The statistics are averaged over \num{100} independent runs.
		We observe that the geodesic \gls{rcg} method achieves a slightly higher objective value with significantly (down to 1/3) lower number of iterations and shorter (down to 1/4) elapsed time than the non-geodesic method.
	...
	\begin{subsection}{Achievable Rate Maximization}
		\begin{subsubsection}{MIMO Point-to-Point Channel}
			...
			An interesting observation is that the rate gap between the optimal \gls{ao} approach in Section \ref{sc:rate_ao} and the shaping-inspired low-complexity solution in Section \ref{sc:rate_lc} narrows as $L$ increases and completely vanishes for a fully-connected \gls{bd}-\gls{ris}.
		...
		\begin{subsubsection}{MIMO Interference Channel}
			\begin{figure}[!t]
				...
				\subfloat[WSR: $(4 \times 64 \times 4)^5$]{
				...
				\\
				\subfloat[WSR: $(4 \times N_\mathrm{S} \times 4)^5$]{
					\label{fg:ic_wsr_sx}
					\resizebox{!}{3.0625cm}{
						\input{../assets/simulation/ic_wsr_sx.tex}
					}
				}
				\subfloat[WSR: $(N_\mathrm{T} \times 128 \times N_\mathrm{R})^5$]{
			...
			The reason is that, for $K$ transmissions of different path loss, interference alignment using a shared passive beamformer alone is challenging especially when the direct channels are significant.
			...
			We expect a narrower performance gap in between when $N_\mathrm{S}$ increases or \gls{ris} coverage area shrinks.
			...
			The reasons discussed in the point-to-point case also apply here.
		\end{subsubsection}
	\end{subsection}
	...
	\begin{subsection}{Practical Constraints}
		\begin{subsubsection}{RIS Symmetry}
			...
			\begin{remark}
				...
				\begin{enumerate}[label=(\roman*)]
					...
					\item \emph{\gls{rcg}-based (e.g., \eqref{eq:shaping_subdiff}, \eqref{eq:gradient_eucl_rate}, \eqref{eq:gradient_eucl_wsr}):} The symmetry constraint is added to the corresponding optimization problems, and one simple strategy is to project the solution to the nearest symmetric point $\mathbf{\Theta} \gets \frac{\mathbf{\Theta} + \mathbf{\Theta}^\mathsf{T}}{2}$ after each iteration. \label{it:projection}
				\end{enumerate}
		...
		\begin{subsubsection}{Channel Estimation Error}
			\begin{figure}[!t]
				...
				\caption{
					Impact of backward/forward channel estimation error and transceiver pairs on the weighted sum-rate of $(4 \times 64 \times 4)^K$ \gls{mimo} interference channel.
				}
			...
			We observe that the proposed joint beamforming design is robust to channel estimation error and thus suitable for practical implementations.
			On the other hand, introducing a \gls{ris} to \gls{ic} systems is helpful to mitigate the rate saturation effect as the number of users increases.
			In the overloaded regime ($K \ge 8$), \gls{bd}-\gls{ris} provides a larger \gls{wsr} slope than \gls{d}-\gls{ris} thanks to its superior shaping capability in aligning the interference subspaces.
...
\begin{section}{Conclusion}
	This paper analyzes the channel shaping capability of \gls{ris} in terms of singular value redistribution.
