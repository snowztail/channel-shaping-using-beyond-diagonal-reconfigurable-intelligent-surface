function [Theta, H] = scatter_rate_ic(H_d, H_f, H_b, W, L, P_n, rho)
	persistent iter;
	if isempty(iter)
	...
	[iter.converge, iter.tolerance, iter.counter, iter.J] = deal(false, 1e-3, 0, rho' * rate_mimo(H, W, P_n));
	while ~iter.converge
		[iter.G_r, iter.D] = deal(G_r, D);
		for g = 1 : G
			S = (g - 1) * L + 1 : g * L;
			S_c = setdiff(1 : length(Theta), S);
			fun = @(Theta_g) rho' * rate_mimo(H_d + pagemtimes(pagemtimes(H_b(:, S, :, :), Theta_g), H_f(S, :, :, :)) + pagemtimes(pagemtimes(H_b(:, S_c, :, :), Theta(S_c, S_c)), H_f(S_c, :, :, :)), W, P_n);
...
function [G_e] = gradient_euclidean(H, H_f, H_b, W, P_n, rho)
	...
	G_e = sum(shiftdim(rho, -2) .* pagemtimes(pagemtimes(pagemrdivide(pagectranspose(H_b), Q), pagemrdivide(F, eye(N_e) + pagemtimes(pagemrdivide(pagectranspose(F), Q), F))), pagemtimes(pageswap(pagectranspose(W)), pageswap(pagectranspose(H_f)) - pagemtimes(pagemrdivide(pagectranspose(H(:, :, logical(eye(K)))), Q), sum(A, 4) - A(:, :, logical(eye(K)))))), 3);
end
