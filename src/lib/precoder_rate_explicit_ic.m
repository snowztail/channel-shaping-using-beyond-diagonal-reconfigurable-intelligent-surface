function [W] = precoder_rate_explicit_ic(H, W, P_t, P_n, rho)
	...
	while ~iter.converge
		...
		iter.converge = (abs(J - iter.J) / iter.J <= iter.tolerance) && all(abs(pagenorm(W, 'fro') .^ 2 - P_t) ./ P_t < iter.tolerance | abs(pagenorm(W, 'fro') .^ 2 - P_t) < iter.tolerance);
		iter.J = J;
