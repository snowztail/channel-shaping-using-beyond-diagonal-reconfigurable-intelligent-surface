function [W] = precoder_rate_ic_bisection(H, W, P_t, P_n, rho)
	...
	while ~iter.converge
		...
		% V = shiftdim(rho, -2) .* pageinv(eye(N_e) - pagemtimes(G, F));
		...
		lambda1 = zeros(1, 1, 1, K);
		for k = 1 : K
			% fun = @(lambda_k) norm((sum(Y(:, :, :, k), 3) + lambda_k * eye(N_t)) \ H(:, :, k, k)' * G(:, :, k)' * V(:, :, k), 'fro') - P_t(k);
			fun = @(lambda_k) norm(mldivide(sum(Y(:, :, :, k), 3) + lambda_k * eye(N_t), H(:, :, k, k)' * G(:, :, k)' * V(:, :, k)), 'fro') ^ 2 - P_t;
			if fun(0) * fun(1e12) > 0
				lambda1(k) = 0;
			else
				lambda1(k) = bisection(fun, 0, 1e12, 1e-6);
			end
		end
		...
		W = pagemldivide(sum(Y, 3) + lambda1 .* eye(N_t), pageswap(pagemtimes(pagemtimes(G, H(:, :, logical(eye(K)))), 'ctranspose', V, 'none')));
