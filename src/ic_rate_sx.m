...
[transmit.antenna, reflect.antenna, receive.antenna, transmit.stream, network.pair] = deal(4, 2 .^ [2, 4, 6, 8], 2, 2, 3);
...
[transmit.power, receive.noise, receive.weight] = deal(db2pow(0), db2pow(-10 : -10 : -40), ones(network.pair, 1));
[channel.pathloss.direct, channel.pathloss.forward, channel.pathloss.backward] = deal(db2pow(0), db2pow(-10), db2pow(-10));
...
for r = 1 : number.realization
	...
	for a = 1 : number.antenna
		...
		for b = 1 : number.bond
			...
			% foo = repmat({dftmtx(reflect.bond(b))}, [reflect.antenna(a) / reflect.bond(b), 1]);
			% reflect.beamformer = blkdiag(foo{:}) / sqrt(reflect.bond(b));
