...
[transmit.antenna, reflect.antenna, receive.antenna, transmit.stream] = deal(4, 2 .^ (4 : 2 : 8), 2, 4);
...
[number.bond, number.power, number.antenna, number.realization] = deal(2, length(transmit.power), length(reflect.antenna), 1e2);
...
for r = 1 : number.realization
	...
	for p = 1 : number.power
		...
		network.wsr.direct(p, r) = sum(network.weight .* rate_mimo(channel.direct, transmit.beamformer, receive.noise), 3);
	end
	% * Have RIS
...
for a = 1 : number.antenna
	handle.wsr.aggregate(1, a) = plot(pow2db(transmit.power), network.wsr.aggregate(:, 1, a) / log(2), 'DisplayName', 'D: $N_\mathrm{S} = ' + string(reflect.antenna(a)) + '$');
	handle.wsr.aggregate(2, a) = plot(pow2db(transmit.power), network.wsr.aggregate(:, 2, a) / log(2), 'DisplayName', 'BD: $N_\mathrm{S} = ' + string(reflect.antenna(a)) + '$');
end
