...
\begin{section}{\glsentryshort{mimo}-\glsentryshort{pc}}
	\begin{subsection}{Channel Power Maximization}
		...
		\end{figure}
		% Fig.~\ref{sm:power_sx} shows that, apart from numerous reflecting elements, a sufficiently large group size can help
		% Fig.~\ref{sm:power_sx} shows that increasing the group size can improve the channel power especially for a large \gls{ris}.
		Fig.~\ref{sm:power_sx} shows that, apart from adding reflecting elements $N^\mathrm{S}$, increasing the group size $L$ also improves the channel power.
	...
	\begin{subsection}{Channel Singular Value Redistribution}
		...
		\begin{figure}
			...
			\label{sm:singular_trend}
		\end{figure}
		Fig.~\ref{sm:singular_trend} illustrates all possible channel singular values achieved by diagonal and symmetry unitary \gls{ris}.
		...
		\begin{subsubsection}{Rank-Deficient Channel}
			...
			Note \gls{bd} \gls{ris} can still provide a higher indirect \gls{snr} as shown in Fig.~\ref{sm:power_sx} and \ref{sm:pc_power_bond}.
		\end{subsubsection}
		...
		\begin{figure}[!t]
			\centering
			\resizebox{0.65\columnwidth}{!}{
				\input{assets/simulation/singular_pareto.tex}
			}
		...
		The Pareto front and evolving trend of channel singular values are shown in Fig.~\ref{sm:singular_pareto} and \ref{sm:pc_singular_bound}.
